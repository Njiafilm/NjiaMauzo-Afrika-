...
Live Activity 24/7 + Wahudumu + Smart Bot + Security
"""
...
ACTIVITY_TEMPLATES = [
    ...
    "🔍 Wahudumu wamepata {product} — {location}",
    ...
    "🌍 Wahudumu wamekagua soko — {product} ({location})",
    "📈 Shughuli za soko za {product} zimeongezeka — {location}",
    "🔎 Wahudumu wanatafuta {product} katika mikoa mbalimbali",
]
...
def _gen_seller_phone(seller_name):
    ...
    rng = random.Random(h)
    prefix = rng.choice(["0755", "0754", "0767", "0782", "0713", "0625", "0692"])
    tail = f"{h % 10000000:07d}"
    return prefix + tail[:7]


def _gen_seller_contacts(seller_name, location=""):
    """Mawasiliano kamili: simu, WhatsApp, email, Telegram, Facebook, Instagram."""
    h = int(hashlib.md5(seller_name.encode()).hexdigest()[:8], 16)
    phone = _gen_seller_phone(seller_name)
    slug = re.sub(r"[^a-z0-9]", "", seller_name.lower())[:12] or "seller"
    loc_slug = re.sub(r"[^a-z0-9]", "", (location or "tz").lower())[:8]
    email = f"{slug}.{loc_slug}@biashara.tz" if loc_slug else f"{slug}@biashara.tz"
    return {
        "seller_phone": phone,
        "seller_contact": phone,
        "seller_whatsapp": phone,
        "seller_email": email,
        "seller_telegram": f"@{slug}_tz",
        "seller_facebook": f"facebook.com/{slug}.tz",
        "seller_instagram": f"instagram.com/{slug}_tz",
    }
...
def ai_search_products(query="", region=None, limit=12):
    ...
    for _ in range(count):
        ...
        contacts = _gen_seller_contacts(seller, loc)
        ...
        full = {
            ...
            "seller_name": seller,
            "seller_id": seller_id,
            "seller_phone": contacts["seller_phone"],
            "seller_contact": contacts["seller_contact"],
            "seller_whatsapp": contacts["seller_whatsapp"],
            "seller_email": contacts["seller_email"],
            "seller_telegram": contacts["seller_telegram"],
            "seller_facebook": contacts["seller_facebook"],
            "seller_instagram": contacts["seller_instagram"],
            "real_price": price,
            "realPrice": price,
...
SELLER_INVITE_TEMPLATES = [
    "📢 Mwaliko kwa wauzaji: Karibu NjiaMauzo Afrika — tangaza biashara yako, tuunganishe na wanunuzi Tanzania na Afrika Mashariki. Biashara inaunganishwa!",
    "🤝 Wauzaji wa {product}: lete bidhaa yako mbele kwenye NjiaMauzo Afrika. Mfumo sahihi wa kuunganisha wauzaji na wanunuzi.",
    "🌟 Karibu muuzaji! Tangaza {product} kutoka {location} kwenye NjiaMauzo Afrika — soko la Afrika Mashariki linakungoja.",
    "📣 Biashara inaunganishwa: NjiaMauzo Afrika inakaribisha wauzaji wote. Ate mbele, tangaza, uonekane na wanunuzi.",
]

def generate_activity_message():
    prod = random.choice(PRODUCTS_CATALOG)
    loc = random.choice(ALL_LOCATIONS)
    # ~25% mwaliko kwa wauzaji; ~75% shughuli za soko
    if random.random() < 0.25:
        template = random.choice(SELLER_INVITE_TEMPLATES)
        msg = template.format(product=prod["title"], location=loc)
        return push_activity(msg, activity_type="invite", extra={"location": loc, "product": prod["title"]})
...
def _bot_search_system(query, region=None, limit=6):
    """Unganisha bot na wahudumu / products_store — rudisha muhtasari bila bei."""
    q = (query or "").strip()
...
def smart_bot_reply(message, history=None, force_lang=None):
    """
    Bot smart — anaungana na wahudumu, anatoa majibu ya mfumo kwanza,
    kisha anataja ada. Lugha: SW / EN / FR (force_lang kutoka kitufe cha lugha).
    """
    msg = (message or "").strip()
    msg_l = msg.lower()
    history = history or []
    lang = force_lang if force_lang in ("sw", "en", "fr") else _detect_lang(msg)
    ...
    def cta_pay(topic=None, lang=lang):
        ...
        if lang == "en":
            ...
            return (
                f"\n\n💡 To get full {t} (price, location, seller contact), tap **PATA MSAADA HAPA**, "
                "pay **TZS 1,000**, and our staff (wahudumu) will send everything on WhatsApp. "
                "I'm still here if you need anything else 😊"
            )
        if lang == "fr":
            return (
                "\n\n💡 Pour les détails complets (prix, lieu, contact vendeur), appuyez sur **PATA MSAADA HAPA**, "
                "payez **TZS 1,000**, et nos agents vous enverront tout sur WhatsApp. Je reste disponible 😊"
            )
        t = topic or "taarifa kamili"
        return (
            f"\n\n💡 Ukitaka {t} (bei, eneo, mawasiliano ya muuzaji), bofya **PATA MSAADA HAPA**, "
            "lipa ada **TZS 1,000**, kisha wahudumu watakutumia kila kitu kwa WhatsApp. "
            "Bado nipo hapa tukiongea 😊"
        )
    ...
    if found:
        ...
        if lang == "en":
            body = (
                f"Okay, I checked with our staff (wahudumu) 🔍 — **{p}** is available "
        ...
        if lang == "fr":
            body = (
                f"D'accord, j'ai vérifié avec nos agents 🔍 — **{p}** est disponible "
        ...
        body = (
            f"Sawa, nimeangalia na **wahudumu** wetu 🔍 — **{p}** inapatikana "
    ...
    if any(w in msg_l for w in ["bei", "price", "ghali", "nafuu", "gharama", "pesa", "ngapi", "cost", "how much", "prix"]):
        if lang == "en":
            return (
                "Full prices are protected for sellers — after you pay **TZS 1,000**, "
                "Our staff will send exact price, location and seller on WhatsApp.\n"
                "Meanwhile, tell me the product name and I'll check availability in the system 😊"
            )
        return (
            "Bei kamili zinalindwa kwa ajili ya wafanyabiashara — baada ya kulipa ada **TZS 1,000**, "
            "wahudumu wanakutumia bei, eneo na muuzaji kwa WhatsApp.\n"
    ...
    if any(w in msg_l for w in ["lipa", "malipo", "ada", "m-pesa", "mpesa", "airtel", "halotel", "payment", "order", "pay"]):
        if lang == "en":
            return (
                ...
                "After confirmation, staff will send full details. Any other question?"
            )
        return (
            "Ada ni **TZS 1,000**, dada.\n"
            ...
            "Baada ya kuthibitisha, wahudumu wanakutumia taarifa kamili. Una swali jingine?"
        )
    ...
    if any(w in msg_l for w in ["ushauri", "mshauri", "wataalamu", "kilimo", "mavuno", "shamba", "kulima", "advice", "farming", "expert"]):
        if lang == "en":
            return (
                "For professional farm advice: tap **Omba Ushauri wa Kitaalamu**, pay TZS 1,000, "
        ...
        return (
            ...
            "lipa TZS 1,000, kisha utaunganishwa na mshauri kupitia WhatsApp **0755 248 789**.\n"
    ...
    if any(w in msg_l for w in ["jinsi", "namna", "how", "kazi", "mfumo", "eleza", "maelezo", "how does", "how it"]):
        if lang == "en":
            return (
                ...
                "3️⃣ Pay TZS 1,000\n"
                "4️⃣ WhatsApp opens\n"
                "5️⃣ Staff send price, location & seller\n"
        ...
        return (
            ...
            "5️⃣ wahudumu wanakutumia bei, eneo na muuzaji\n"
    ...
    if any(w in msg_l for w in ["asante", "thanks", "thank you", "shukrani", "nashukuru", "merci"]):
        ...
        return "Karibu sana! 😊 Rudi wakati wowote — nipo hapa 24/7."
    ...
    if any(w in msg_l for w in ["wewe nani", "nani wewe", "bot", "robot", "who are you", "your name"]):
        if lang == "en":
            return (
                ...
                "I search products with our staff (wahudumu), explain payments, and connect you to advisors. What do you need?"
            )
        return (
            "Mimi ni msaidizi wa NjiaMauzo Afrika — binti anayekuhudumia 24/7 😊 "
            "Natafuta mazao kwa wahudumu, ninaeleza malipo, na nakuunganisha na washauri. Unahitaji nini?"
        )
...
def get_products():
    ...
    for p in products_store[:30]:
        if unlocked:
            ...
            loc = p.get("location", "") or "Dar es Salaam"
            contacts = _gen_seller_contacts(sname, loc) if sname else {}
            phone = p.get("seller_phone") or p.get("seller_contact") or contacts.get("seller_phone", "")
            tcost = p.get("transport_cost")
            if tcost is None:
                tcost, tnote = _estimate_transport(loc, p.get("unit", "gunia"))
            else:
                tnote = p.get("transport_note") or f"Makadirio ya usafiri kutoka {loc} hadi eneo lako"
            products.append({
                "id": p["id"],
                ...
                "seller_phone": phone,
                "seller_contact": phone,
                "seller_whatsapp": p.get("seller_whatsapp") or contacts.get("seller_whatsapp", phone),
                "seller_email": p.get("seller_email") or contacts.get("seller_email", ""),
                "seller_telegram": p.get("seller_telegram") or contacts.get("seller_telegram", ""),
                "seller_facebook": p.get("seller_facebook") or contacts.get("seller_facebook", ""),
                "seller_instagram": p.get("seller_instagram") or contacts.get("seller_instagram", ""),
...
def ai_products():
    ...
    if q:
        push_activity(
            f"🔍 Wahudumu wametafuta '{q}' katika mikoa ya Tanzania na Afrika Mashariki — {len(results)} matokeo",
    ...
    for p in results:
        if unlocked:
            public_results.append({
                ...
                "seller_whatsapp": p.get("seller_whatsapp") or p.get("seller_phone") or "",
                "seller_email": p.get("seller_email", ""),
                "seller_telegram": p.get("seller_telegram", ""),
                "seller_facebook": p.get("seller_facebook", ""),
                "seller_instagram": p.get("seller_instagram", ""),
                "real_price": p.get("real_price", 0),
...
def notify_customer_after_payment(phone, order_id, query_or_product):
    ...
    thank_you = (
        ...
        "Malipo yako yamepokelewa — Wahudumu wanatafuta taarifa unazohitaji, "
...
def payment_request():
    ...
    return jsonify({
        ...
        "order_id": order_id,
        "status": "PENDING",
        "hint": f"Wahudumu wataandaa taarifa za {sample.get('jina', 'bidhaa')} baada ya kuthibitisha malipo.",
        "unlocked": True,
...
def bot_chat():
    ...
    # Prefer client language toggle when message is ambiguous
    client_lang = str(data.get("lang") or "").strip().lower()
    force = client_lang if client_lang in ("en", "sw", "fr") else None
    reply = smart_bot_reply(message, history=history, force_lang=force)
    return jsonify({"reply": reply, "lang": force or _detect_lang(message)})
