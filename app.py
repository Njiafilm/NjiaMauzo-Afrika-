...
# Upload storage lazima itangazwe kabla ya routes/blocks zinazotumia UPLOAD_DIR
# wakati app.py inapoload. Hii inazuia NameError kwenye Render/Gunicorn.
...
app.config.update(
    ...
    MAX_CONTENT_LENGTH=200 * 1024 * 1024,  # 200MB uploads
)
...
# Vyanzo vya rejea vya Nyaraka. Admin anaweza kuongeza/kufuta vyanzo kupitia Admin Room.
DEFAULT_DOCLIB_SOURCES = {
    "biashara": [
        {"title": "IFC — Small Business", "url": "https://www.ifc.org/en/what-we-do/smes"},
        {"title": "ILO — Small and Medium Enterprises", "url": "https://www.ilo.org/topics-and-sectors/small-and-medium-enterprises"},
    ],
    "elimu": [
        {"title": "UNESCO — Education", "url": "https://www.unesco.org/en/education"},
    ],
    "mbegu": [
        {"title": "FAO — Seeds", "url": "https://www.fao.org/seeds/en/"},
    ],
    "usimamizi": [
        {"title": "FAO — Land & Water", "url": "https://www.fao.org/land-water/en/"},
    ],
    "ushauri_kilimo": [
        {"title": "FAO — Agriculture", "url": "https://www.fao.org/agriculture/en/"},
    ],
}

def _doclib_sources_path():
    path = UPLOAD_DIR / "doclib" / "sources.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    return path

def _load_doclib_sources():
    path = _doclib_sources_path()
    try:
        ...
        data = {}
    merged = {k: list(v) for k, v in DEFAULT_DOCLIB_SOURCES.items()}
    for tid, items in data.items():
        if tid in DOCLIB_TOPICS and isinstance(items, list):
            clean = []
            for item in items[:12]:
                if isinstance(item, dict) and str(item.get("url", "")).startswith(("http://", "https://")):
                    clean.append({"title": str(item.get("title") or "Chanzo cha rejea")[:120], "url": str(item.get("url"))[:500]})
            merged[tid] = clean
    return merged

def _save_doclib_sources(data):
    path = _doclib_sources_path()
...
def api_doclib_topics():
    ...
    for tid, label in DOCLIB_TOPICS.items():
        ...
        out.append({
            "id": tid,
            "label": label,
            "has_document": path.exists(),
            # Vyanzo ni vya Admin Room pekee; public haipati links hizi.
            "sources": sources.get(tid, []) if session.get("is_admin") else []
        })
    return jsonify({"success": True, "topics": out})


@app.route("/api/doclib/sources/<topic_id>", methods=["GET"])
def api_doclib_sources(topic_id):
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Vyanzo vya masomo vinapatikana ndani ya Admin Room pekee."}), 403
    if topic_id not in DOCLIB_TOPICS:
        return jsonify({"success": False, "message": "Mada haipo."}), 404
    return jsonify({"success": True, "topic_id": topic_id, "sources": _load_doclib_sources().get(topic_id, [])})


@app.route("/api/admin/doclib/source", methods=["POST"])
def api_admin_doclib_source():
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    data = request.get_json(silent=True) or {}
    ...
    if not title or not url.startswith(("https://", "http://")):
        return jsonify({"success": False, "message": "Weka jina na link sahihi ya chanzo (http/https)."}), 400
    sources = _load_doclib_sources()
    items = sources.setdefault(topic_id, [])
    if not any(x.get("url") == url for x in items):
        items.append({"title": title, "url": url})
    _save_doclib_sources(sources)
    return jsonify({"success": True, "message": "Chanzo kimeongezwa.", "sources": sources[topic_id]})


@app.route("/api/admin/doclib/source/delete", methods=["POST"])
def api_admin_doclib_source_delete():
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    data = request.get_json(silent=True) or {}
    topic_id = str(data.get("topic_id") or "").strip()
    ...
    if topic_id not in DOCLIB_TOPICS:
        return jsonify({"success": False, "message": "Mada si sahihi."}), 400
    sources = _load_doclib_sources()
    sources[topic_id] = [x for x in sources.get(topic_id, []) if x.get("url") != url]
    _save_doclib_sources(sources)
    return jsonify({"success": True, "message": "Chanzo kimeondolewa.", "sources": sources[topic_id]})
...
def _import_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS import_products (
        ...
        shipper_name TEXT, shipper_contact TEXT,
        description TEXT,
...
REELS_DB = REELS_DIR / "reels.json"
REELS_LOCK = threading.Lock()
ALLOWED_REEL_EXTENSIONS = {".mp4", ".webm", ".mov", ".m4v"}

def _load_reels():
    try:
        data = json.loads(REELS_DB.read_text(encoding="utf-8")) if REELS_DB.exists() else []
        return data if isinstance(data, list) else []
    except Exception:
        return []

def _save_reels(items):
    tmp = REELS_DB.with_suffix(".tmp")
    tmp.write_text(json.dumps(items, ensure_ascii=False, indent=2), encoding="utf-8")
    tmp.replace(REELS_DB)

def _reels_public(items):
    return [
        {
            "id": int(x.get("id", 0)),
...
def api_reels_public():
    with REELS_LOCK:
        items = _load_reels()
    items.sort(key=lambda x: x.get("created", ""), reverse=True)
    return jsonify({"success": True, "reels": _reels_public(items[:50])})

@app.route("/api/admin/reels", methods=["GET"])
def api_admin_reels_list():
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    with REELS_LOCK:
        items = _load_reels()
    items.sort(key=lambda x: x.get("created", ""), reverse=True)
    return jsonify({"success": True, "reels": _reels_public(items[:100])})

@app.route("/api/admin/reels", methods=["POST"])
def api_admin_reels_create():
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    ok, err = _require_csrf()
    if not ok:
    ...
    if not f or not f.filename:
        return jsonify({"success": False, "message": "Chagua video kwanza."}), 400

    ext = Path(f.filename).suffix.lower()
    if ext not in ALLOWED_REEL_EXTENSIONS:
        return jsonify({"success": False, "message": "Aina ya video hairuhusiwi. Tumia MP4, WebM, MOV au M4V."}), 400

    # Reject obviously oversized uploads before writing.
    content_length = request.content_length or 0
    if content_length > 200 * 1024 * 1024:
        return jsonify({"success": False, "message": "Video imezidi kikomo cha 200MB."}), 413

    safe_name = secure_filename(Path(f.filename).stem) or "reel"
    filename = f"reel_{secrets.token_hex(8)}_{safe_name[:60]}{ext}"
    dest = REELS_DIR / filename
    f.save(str(dest))

    title = (request.form.get("title") or safe_name or "NjiaMauzo Reel").strip()[:180]
    ...
    return jsonify({"success": True, "reel": _reels_public([item])[0], "message": "🎬 Reel imepakiwa na iko tayari kuonekana kwa umma."})

@app.route("/api/admin/reels/<int:reel_id>", methods=["DELETE"])
def api_admin_reels_delete(reel_id):
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    ok, err = _require_csrf()
    if not ok:
    ...
    with REELS_LOCK:
        ...
        keep = []
        for item in items:
            if int(item.get("id", 0)) == reel_id:
                removed = item
            else:
                keep.append(item)
        if removed is None:
            return jsonify({"success": False, "message": "Reel haipatikani."}), 404
        _save_reels(keep)

    filename = removed.get("filename")
    if filename:
        try:
            (REELS_DIR / Path(filename).name).unlink(missing_ok=True)
        except Exception:
            pass
...
def api_admin_import_product_create():
    ...
    conn.execute(
        """INSERT INTO import_products(category,title,source_country,image_url,estimated_price_tzs,
               shipping_cost_tzs,customs_duty_note,seller_name,seller_contact,shipper_name,shipper_contact,
               description,active,created_at)
           VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1,?)""",
        (
            (d.get("category") or "import").strip(), title, (d.get("source_country") or "").strip(),
            image_url,
            int(d.get("estimated_price_tzs") or 0) or None,
            int(d.get("shipping_cost_tzs") or 0) or None,
            (d.get("customs_duty_note") or "").strip(),
            (d.get("seller_name") or "").strip(), (d.get("seller_contact") or "").strip(),
            (d.get("shipper_name") or "").strip(), (d.get("shipper_contact") or "").strip(),
...
def api_ads_list():
    ...
    with ADS_LOCK:
        active = [
            ...
            if a.get("active", True) and (not a.get("ends_at") or a.get("ends_at") > now_iso)
        ]
