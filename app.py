...
# Company support contacts (create these inboxes with your email provider/domain).
SUPPORT_EMAIL = os.environ.get("SUPPORT_EMAIL", "support@njiamauzoafrika.com").strip()
INFO_EMAIL = os.environ.get("INFO_EMAIL", "info@njiamauzoafrika.com").strip()
CHAT_ADMIN_PRESENCE_LOCK = threading.Lock()
CHAT_ADMIN_ACTIVE = False
CHAT_ADMIN_ACTIVE_AT = None
...
# ================= ADMIN MULTI-ACCOUNT / UONGOZI =================
# Akaunti 15: Mkurugenzi Mkuu 1 + Mhasibu 1 + Admins 13.
# Credentials zinaweza kubadilishwa kwa environment variables; password hazitolewi kwa API.
ADMIN_ACCOUNT_SPECS = [
    {"username": os.environ.get("ADMIN_DIRECTOR_USER", "MkurugenziMkuu"), "password": os.environ.get("ADMIN_DIRECTOR_PASS", "NjiaMauzo.Director@2026"), "name": "Mkurugenzi Mkuu", "role": "director", "email": os.environ.get("ADMIN_DIRECTOR_EMAIL", "director@njiamauzoafrika.com")},
    {"username": os.environ.get("ADMIN_ACCOUNTANT_USER", "Mhasibu"), "password": os.environ.get("ADMIN_ACCOUNTANT_PASS", "NjiaMauzo.Accountant@2026"), "name": "Mhasibu", "role": "accountant", "email": os.environ.get("ADMIN_ACCOUNTANT_EMAIL", "accountant@njiamauzoafrika.com")},
]
for _i in range(1, 14):
    ADMIN_ACCOUNT_SPECS.append({
        "username": os.environ.get(f"ADMIN_{_i}_USER", f"Admin{_i}"),
        "password": os.environ.get(f"ADMIN_{_i}_PASS", f"NjiaMauzo.Admin{_i}@2026"),
        "name": os.environ.get(f"ADMIN_{_i}_NAME", f"Admin { _i}"),
        "role": "admin",
        "email": os.environ.get(f"ADMIN_{_i}_EMAIL", f"admin{_i}@njiamauzoafrika.com"),
    })

ADMIN_ACCOUNTS_LOCK = threading.RLock()
...
def _admin_accounts_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS admin_reports (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        admin_id INTEGER,
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_admin_reports_created ON admin_reports(created_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_admin_audit_created ON admin_audit_log(created_at)")
    conn.commit()
    return conn

def _pw_hash(password):
    return hashlib.sha256(("njiamauzo-admin-v2|" + str(password)).encode()).hexdigest()

def _seed_admin_accounts():
    with ADMIN_ACCOUNTS_LOCK:
        conn = _admin_accounts_db()
        now = datetime.utcnow().isoformat() + "Z"
        for spec in ADMIN_ACCOUNT_SPECS:
            row = conn.execute("SELECT id FROM admin_accounts WHERE username=?", (spec["username"],)).fetchone()
            if not row:
                conn.execute("INSERT INTO admin_accounts(username,password_hash,display_name,role,email,active,created_at) VALUES(?,?,?,?,?,1,?)", (spec["username"], _pw_hash(spec["password"]), spec["name"], spec["role"], spec["email"], now))
            else:
                conn.execute("UPDATE admin_accounts SET display_name=?,role=?,email=? WHERE username=?", (spec["name"], spec["role"], spec["email"], spec["username"]))
        conn.commit(); conn.close()

_seed_admin_accounts()

def _current_admin_role():
    return session.get("admin_role") or (session.get("user") or {}).get("role") or "admin"

def _admin_can_manage():
    return _current_admin_role() == "director"

def _admin_can_finance():
    return _current_admin_role() in ("director", "accountant")

def _audit(action, target="", details=""):
    try:
        conn = _admin_accounts_db()
        uid = session.get("admin_id")
        actor = (session.get("user") or {}).get("name") or "Admin"
        conn.execute("INSERT INTO admin_audit_log(actor_id,actor,action,target,details,created_at) VALUES(?,?,?,?,?,?)", (uid, actor, action, str(target or ""), str(details or ""), datetime.utcnow().isoformat()+"Z"))
        conn.commit(); conn.close()
    except Exception:
        pass
...
def api_admin_login():
    """Login ya akaunti 15 za Admin. Temporary mode humuingiza Mkurugenzi Mkuu."""
    ip = _client_ip()
    locked, wait = _admin_is_locked(ip)
    if locked:
        return jsonify({"success": False, "message": f"Jaribio nyingi. Subiri sekunde {wait} kisha ujaribu tena.", "locked_seconds": wait}), 429
    data = request.get_json(silent=True) or {}
    username = (data.get("username") or "").strip()
    password = data.get("password") or ""
    spec = None
    if _admin_temp_open():
        spec = next((x for x in ADMIN_ACCOUNT_SPECS if x["role"] == "director"), ADMIN_ACCOUNT_SPECS[0])
        _admin_clear_attempts(ip)
    else:
        conn = _admin_accounts_db()
        row = conn.execute("SELECT * FROM admin_accounts WHERE username=? AND active=1", (username,)).fetchone()
        conn.close()
        if row and secrets.compare_digest(str(row["password_hash"]), _pw_hash(password)):
            spec = dict(row)
            _admin_clear_attempts(ip)
        else:
            # Backward compatibility with the original single ADMIN_USER/ADMIN_PASS.
            if username and password and secrets.compare_digest(username, ADMIN_USER) and secrets.compare_digest(password, ADMIN_PASS):
                spec = {"id": 0, "username": ADMIN_USER, "name": "Admin", "role": "admin", "email": "admin@njiamauzo.tz"}
                _admin_clear_attempts(ip)
            else:
                rowf = _admin_register_fail(ip)
                left = max(0, ADMIN_MAX_ATTEMPTS - int(rowf.get("count") or 0))
                return jsonify({"success": False, "message": "Jina la mtumiaji au password si sahihi.", "attempts_left": left}), 401
    ...
    name = spec.get("display_name") or spec.get("name") or "Admin"
    email = spec.get("email") or ""
    session["admin_id"] = int(spec.get("id") or 0)
    session["admin_username"] = spec.get("username") or username
    ...
    try:
        if int(spec.get("id") or 0):
            conn = _admin_accounts_db(); conn.execute("UPDATE admin_accounts SET last_login_at=? WHERE id=?", (datetime.utcnow().isoformat()+"Z", int(spec["id"]))); conn.commit(); conn.close()
        _audit("login", name, f"role={role}")
    except Exception: pass
    return jsonify({"success": True, "message": "Admin umeingia.", "admin_mode": True, "role": role, "role_label": "Mkurugenzi Mkuu" if role=="director" else ("Mhasibu" if role=="accountant" else "Admin"), "temporary_open": _admin_temp_open(), "seconds_left": _admin_temp_seconds_left(), "csrf_token": csrf})
...
def api_admin_status():
    ...
    return jsonify({
        ...
        "user": session.get("user"),
        "role": role,
        "role_label": "Mkurugenzi Mkuu" if role=="director" else ("Mhasibu" if role=="accountant" else "Admin"),
...
def _support_db_init(conn):
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_support_requests_session ON support_requests(session_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_support_requests_status ON support_requests(status)")
    conn.commit()


@app.route("/api/chat/admin-status", methods=["GET"])
def api_chat_admin_status():
    with CHAT_ADMIN_PRESENCE_LOCK:
        return jsonify({"success": True, "active": bool(CHAT_ADMIN_ACTIVE), "active_at": CHAT_ADMIN_ACTIVE_AT})


@app.route("/api/admin/chat/presence", methods=["POST"])
def api_admin_chat_presence():
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    ok, err = _require_csrf()
    if not ok:
    ...
    global CHAT_ADMIN_ACTIVE, CHAT_ADMIN_ACTIVE_AT
    with CHAT_ADMIN_PRESENCE_LOCK:
        CHAT_ADMIN_ACTIVE = active
        CHAT_ADMIN_ACTIVE_AT = datetime.utcnow().isoformat() + "Z" if active else None
    return jsonify({"success": True, "active": active, "message": "Admin yuko ACTIVE." if active else "Admin amewekwa OFFLINE."})
...
def api_admin_chat_send():
    ...
    message = (data.get("message") or "").strip()[:500]
    if not message:
        return jsonify({"success": False, "message": "Andika ujumbe."}), 400
    now = datetime.utcnow().isoformat() + "Z"
    with CHAT_LOCK:
        conn = _analytics_db(); _chat_db_init(conn)
        conn.execute("INSERT INTO chat_messages(phone,name,message,status,created_at) VALUES(?,?,?,?,?)", ("ADMIN", "Admin", message, "ok", now))
        conn.commit(); conn.close()
    return jsonify({"success": True, "message": "Ujumbe umetumwa kwenye Chat."})


@app.route("/api/support/request", methods=["POST"])
@rate_limit("support_request", max_attempts=12, window_seconds=300)
def api_support_request():
    data = request.get_json(silent=True) or {}
    issue = (data.get("issue") or "").strip()[:800]
    location = (data.get("location") or "").strip()[:200]
    product_id = str(data.get("product_id") or "")[:80]
    product_title = (data.get("product_title") or "Bidhaa").strip()[:200]
    if not issue or not location:
        return jsonify({"success": False, "message": "Tafadhali taja changamoto na eneo unaloishi."}), 400
    sid = session.get("support_session_id")
    if not sid:
        sid = secrets.token_urlsafe(18); session["support_session_id"] = sid
    now = datetime.utcnow().isoformat() + "Z"
    with CHAT_ADMIN_PRESENCE_LOCK:
        active = bool(CHAT_ADMIN_ACTIVE)
    with CHAT_LOCK:
        conn = _analytics_db(); _support_db_init(conn)
        cur = conn.execute("INSERT INTO support_requests(session_id,product_id,product_title,issue,location,status,created_at) VALUES(?,?,?,?,?,?,?)", (sid,product_id,product_title,issue,location,"pending",now))
        rid = cur.lastrowid
        conn.commit(); conn.close()
    return jsonify({"success": True, "request_id": rid, "admin_active": active, "message": "Ombi limepokelewa."})


@app.route("/api/support/request/<int:rid>", methods=["GET"])
def api_support_request_status(rid):
    sid = session.get("support_session_id")
    if not sid:
        return jsonify({"success": False, "message": "Ombi halijapatikana."}), 404
    with CHAT_LOCK:
        conn = _analytics_db(); _support_db_init(conn)
        row = conn.execute("SELECT id,product_id,product_title,issue,location,status,admin_message,created_at,accepted_at FROM support_requests WHERE id=? AND session_id=?", (rid,sid)).fetchone()
        conn.close()
    if not row:
        return jsonify({"success": False, "message": "Ombi halijapatikana."}), 404
...
def api_admin_support_requests():
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    with CHAT_LOCK:
        conn = _analytics_db(); _support_db_init(conn)
        rows = conn.execute("SELECT id,session_id,product_id,product_title,issue,location,status,admin_message,created_at,accepted_at FROM support_requests ORDER BY id DESC LIMIT 100").fetchall()
        conn.close()
    return jsonify({"success": True, "requests": [dict(r) for r in rows]})


@app.route("/api/admin/support/requests/<int:rid>/accept", methods=["POST"])
def api_admin_support_accept(rid):
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    ok, err = _require_csrf()
    if not ok:
    ...
    with CHAT_LOCK:
        conn = _analytics_db(); _support_db_init(conn)
        row = conn.execute("SELECT id,session_id FROM support_requests WHERE id=?", (rid,)).fetchone()
        if not row:
            conn.close(); return jsonify({"success": False, "message": "Ombi halijapatikana."}), 404
        conn.execute("UPDATE support_requests SET status='accepted',admin_message=?,accepted_at=? WHERE id=?", ("Mhudumu yuko tayari kukusaidia.", now, rid))
        conn.commit(); conn.close()
    return jsonify({"success": True, "message": "Ombi limekubaliwa.", "request_id": rid})


@app.route("/api/support/contact", methods=["GET"])
def api_support_contact():
    return jsonify({"success": True, "support_email": SUPPORT_EMAIL, "info_email": INFO_EMAIL, "whatsapp": "255755248789"})
...
def api_reels_list():
    conn = _analytics_db()
    conn.execute("CREATE TABLE IF NOT EXISTS reels (id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT,description TEXT,video_url TEXT,created_at TEXT,active INTEGER DEFAULT 1)")
    rows = conn.execute("SELECT * FROM reels WHERE active=1 ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()
    return jsonify({"success": True, "reels": [dict(r) for r in rows]})
...
def api_reels_create():
    ...
    title = (request.form.get("title") or "Reel").strip()[:180]
    desc = (request.form.get("description") or "").strip()[:1000]
    if not f or not f.filename: return jsonify({"success":False,"message":"Chagua video kwanza."}),400
    ext = Path(f.filename).suffix.lower()
    if ext not in {".mp4",".webm",".mov",".m4v"}: return jsonify({"success":False,"message":"Video hii hairuhusiwi."}),400
    dest=UPLOAD_DIR/"reels"; dest.mkdir(parents=True,exist_ok=True)
    fname=f"reel_{secrets.token_hex(10)}{ext}"; f.save(str(dest/fname))
    url=f"/static/uploads/reels/{fname}"; now=datetime.utcnow().isoformat()+"Z"
    conn=_analytics_db(); conn.execute("CREATE TABLE IF NOT EXISTS reels (id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT,description TEXT,video_url TEXT,created_at TEXT,active INTEGER DEFAULT 1)")
    cur=conn.execute("INSERT INTO reels(title,description,video_url,created_at,active) VALUES(?,?,?,?,1)",(title,desc,url,now)); rid=cur.lastrowid; conn.commit(); conn.close(); _audit("reel_create", rid, title)
    return jsonify({"success":True,"reel":{"id":rid,"title":title,"description":desc,"url":url,"video_url":url,"created_at":now},"message":"Reel imepakiwa."})

@app.route("/api/reels/<int:rid>", methods=["DELETE"])
def api_reels_delete(rid):
    ok, err = _require_admin()
    if not ok: return err
    ok, err = _require_csrf()
    if not ok: return err
    conn=_analytics_db(); conn.execute("CREATE TABLE IF NOT EXISTS reels (id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT,description TEXT,video_url TEXT,created_at TEXT,active INTEGER DEFAULT 1)")
    row=conn.execute("SELECT * FROM reels WHERE id=?",(rid,)).fetchone()
    if not row: conn.close(); return jsonify({"success":False,"message":"Reel haipo."}),404
    conn.execute("UPDATE reels SET active=0 WHERE id=?",(rid,)); conn.commit(); conn.close(); _audit("reel_delete", rid, row["title"])
    return jsonify({"success":True,"message":"Reel imefutwa."})

# ================= ADMIN ACCOUNTS / REPORTS / AUDIT =================
@app.route("/api/admin/accounts", methods=["GET"])
def api_admin_accounts():
    ok, err = _require_admin()
    if not ok: return err
    conn=_admin_accounts_db(); rows=conn.execute("SELECT id,username,display_name,role,email,active,last_login_at,created_at FROM admin_accounts ORDER BY id").fetchall(); conn.close()
    return jsonify({"success":True,"accounts":[dict(r) for r in rows],"current_admin":session.get("admin_id",0),"role":_current_admin_role()})

@app.route("/api/admin/accounts/<int:aid>", methods=["GET"])
def api_admin_account_view(aid):
    ok, err = _require_admin()
    if not ok: return err
    if not _admin_can_manage(): return jsonify({"success":False,"message":"Ni Mkurugenzi Mkuu pekee anayeruhusiwa kukagua akaunti hii."}),403
    conn=_admin_accounts_db(); row=conn.execute("SELECT id,username,display_name,role,email,active,last_login_at,created_at FROM admin_accounts WHERE id=?",(aid,)).fetchone(); conn.close()
    if not row:return jsonify({"success":False,"message":"Akaunti haipo."}),404
    _audit("private_account_review", aid, "Director account review")
    return jsonify({"success":True,"account":dict(row)})

@app.route("/api/admin/accounts", methods=["POST"])
def api_admin_account_create():
    """Create Director/Accountant/Admin accounts. Only the Director can create accounts; maximum 15."""
    ok, err = _require_admin()
    if not ok: return err
    if not _admin_can_manage():
        return jsonify({"success": False, "message": "Mkurugenzi Mkuu pekee ndiye anaweza kutengeneza akaunti mpya."}), 403
    ok, err = _require_csrf()
    if not ok: return err
    data = request.get_json(silent=True) or {}
    username = str(data.get("username") or "").strip()[:80]
    password = str(data.get("password") or "")
    display_name = str(data.get("display_name") or "").strip()[:120]
    email = str(data.get("email") or "").strip()[:180]
    role = str(data.get("role") or "admin").strip().lower()
    if role not in ("admin", "accountant", "director"):
        return jsonify({"success": False, "message": "Nafasi si sahihi."}), 400
    if not username or not display_name or len(password) < 10:
        return jsonify({"success": False, "message": "Weka username, jina kamili na password yenye angalau herufi 10."}), 400
    with ADMIN_ACCOUNTS_LOCK:
        conn = _admin_accounts_db()
        count = conn.execute("SELECT COUNT(*) c FROM admin_accounts").fetchone()["c"]
        if count >= 15:
            conn.close()
            return jsonify({"success": False, "message": "Kikomo cha akaunti 15 kimefikiwa."}), 409
        if conn.execute("SELECT id FROM admin_accounts WHERE username=?", (username,)).fetchone():
            conn.close()
            return jsonify({"success": False, "message": "Username hiyo tayari ipo."}), 409
        if role == "director" and conn.execute("SELECT id FROM admin_accounts WHERE role='director'").fetchone():
            conn.close()
            return jsonify({"success": False, "message": "Akaunti ya Mkurugenzi Mkuu tayari ipo."}), 409
        if role == "accountant" and conn.execute("SELECT id FROM admin_accounts WHERE role='accountant'").fetchone():
            conn.close()
            return jsonify({"success": False, "message": "Akaunti ya Mhasibu tayari ipo."}), 409
        now = datetime.utcnow().isoformat() + "Z"
        cur = conn.execute("INSERT INTO admin_accounts(username,password_hash,display_name,role,email,active,created_at) VALUES(?,?,?,?,?,1,?)",
                           (username, _pw_hash(password), display_name, role, email, now))
        aid = cur.lastrowid
        conn.commit(); conn.close()
    _audit("create_account", aid, f"username={username}; role={role}")
    return jsonify({"success": True, "message": "Akaunti imetengenezwa na imewekwa ACTIVE.", "account": {"id": aid, "username": username, "display_name": display_name, "role": role, "email": email, "active": 1}})

@app.route("/api/admin/my-room", methods=["GET"])
def api_admin_my_room():
    """Personal room for the currently logged-in admin."""
    ok, err = _require_admin()
    if not ok: return err
    aid = int(session.get("admin_id") or 0)
    conn = _admin_accounts_db()
    row = conn.execute("SELECT id,username,display_name,role,email,active,last_login_at,created_at FROM admin_accounts WHERE id=?", (aid,)).fetchone()
    reports = conn.execute("SELECT id,subject,message,status,created_at FROM admin_reports WHERE admin_id=? ORDER BY id DESC LIMIT 20", (aid,)).fetchall()
    conn.close()
    if not row:
        return jsonify({"success": False, "message": "Chumba cha admin hakipatikani."}), 404
    return jsonify({"success": True, "room": {"account": dict(row), "reports": [dict(x) for x in reports], "role": _current_admin_role()}})

@app.route("/api/admin/accounts/<int:aid>/room", methods=["GET"])
def api_admin_account_room(aid):
    """Director may open another admin's room; access is explicitly logged."""
    ok, err = _require_admin()
    if not ok: return err
    if not _admin_can_manage():
        return jsonify({"success": False, "message": "Mkurugenzi Mkuu pekee ndiye anaweza kufungua chumba cha Admin mwingine."}), 403
    conn = _admin_accounts_db()
    row = conn.execute("SELECT id,username,display_name,role,email,active,last_login_at,created_at FROM admin_accounts WHERE id=?", (aid,)).fetchone()
    reports = conn.execute("SELECT id,subject,message,status,created_at FROM admin_reports WHERE admin_id=? ORDER BY id DESC LIMIT 50", (aid,)).fetchall()
    conn.close()
    if not row: return jsonify({"success": False, "message": "Akaunti haipo."}), 404
    _audit("open_admin_room", aid, "Director opened admin room")
    return jsonify({"success": True, "room": {"account": dict(row), "reports": [dict(x) for x in reports]}})

@app.route("/api/admin/accounts/<int:aid>/reset-password", methods=["POST"])
def api_admin_account_reset(aid):
    ok, err=_require_admin()
    if not ok:return err
    if not _admin_can_manage():return jsonify({"success":False,"message":"Mkurugenzi Mkuu pekee."}),403
    ok,err=_require_csrf()
    if not ok:return err
    data=request.get_json(silent=True) or {}; pw=str(data.get("new_password") or "")
    if len(pw)<10:return jsonify({"success":False,"message":"Password lazima iwe na angalau herufi 10."}),400
    conn=_admin_accounts_db(); row=conn.execute("SELECT username FROM admin_accounts WHERE id=?",(aid,)).fetchone()
    if not row:conn.close();return jsonify({"success":False,"message":"Akaunti haipo."}),404
    conn.execute("UPDATE admin_accounts SET password_hash=? WHERE id=?",(_pw_hash(pw),aid));conn.commit();conn.close();_audit("reset_password",aid,"Director reset admin password")
    return jsonify({"success":True,"message":"Password imebadilishwa."})

@app.route("/api/admin/accounts/<int:aid>/toggle", methods=["POST"])
def api_admin_account_toggle(aid):
    ok,err=_require_admin()
    if not ok:return err
    if not _admin_can_manage():return jsonify({"success":False,"message":"Mkurugenzi Mkuu pekee."}),403
    ok,err=_require_csrf()
    if not ok:return err
    conn=_admin_accounts_db(); row=conn.execute("SELECT username,active,role FROM admin_accounts WHERE id=?",(aid,)).fetchone()
    if not row:conn.close();return jsonify({"success":False,"message":"Akaunti haipo."}),404
    if aid==int(session.get("admin_id") or 0) or row["role"]=="director":conn.close();return jsonify({"success":False,"message":"Akaunti hii haiwezi kuzimwa hapa."}),400
    active=0 if int(row["active"]) else 1; conn.execute("UPDATE admin_accounts SET active=? WHERE id=?",(active,aid));conn.commit();conn.close();_audit("toggle_account",aid,f"active={active}")
    return jsonify({"success":True,"message":"Akaunti imesasishwa."})
...
def api_admin_reports():
    ...
    if request.method=="POST":
        ok,err=_require_csrf()
        if not ok:conn.close();return err
        data=request.get_json(silent=True) or {}; subject=str(data.get("subject") or "").strip()[:180]; message=str(data.get("message") or "").strip()[:5000]
        if not subject or not message:conn.close();return jsonify({"success":False,"message":"Weka kichwa na taarifa."}),400
        conn.execute("INSERT INTO admin_reports(admin_id,subject,message,status,created_at) VALUES(?,?,?,?,?)",(session.get("admin_id"),subject,message,"pending",datetime.utcnow().isoformat()+"Z"));conn.commit();conn.close();_audit("submit_report",session.get("admin_id"),subject)
        return jsonify({"success":True,"message":"Taarifa imetumwa kwa Mhasibu na Mkurugenzi."})
    if _admin_can_finance():
        rows=conn.execute("SELECT r.*,a.username,a.display_name,a.role FROM admin_reports r LEFT JOIN admin_accounts a ON a.id=r.admin_id ORDER BY r.id DESC LIMIT 200").fetchall()
    else:
        rows=conn.execute("SELECT r.*,a.username,a.display_name,a.role FROM admin_reports r LEFT JOIN admin_accounts a ON a.id=r.admin_id WHERE r.admin_id=? ORDER BY r.id DESC LIMIT 100",(session.get("admin_id"),)).fetchall()
    conn.close(); return jsonify({"success":True,"reports":[dict(r) for r in rows]})

@app.route("/api/admin/audit-log", methods=["GET"])
def api_admin_audit_log():
    ok,err=_require_admin()
    if not ok:return err
    if not _admin_can_finance():return jsonify({"success":False,"message":"Ni Mkurugenzi Mkuu au Mhasibu pekee."}),403
    conn=_admin_accounts_db(); rows=conn.execute("SELECT * FROM admin_audit_log ORDER BY id DESC LIMIT 300").fetchall();conn.close()
