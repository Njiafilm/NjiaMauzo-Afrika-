...
# ============================================================
# MARKET STATISTICS  (auto stats per category / crop type,
# East Africa marketplace overview)
# ============================================================
...
def market_stats():
    ...
    total_likes = sum(int(item.get("likes") or 0) for item in items)

    categories = {}

    for item in items:

        category = item.get("category") or "nyingine"
        ...
        bucket["count"] += 1
        bucket["prices"].append(float(item.get("real_price") or 0))
        bucket["likes"] += int(item.get("likes") or 0)

        location = item.get("location") or "Haijulikani"

        bucket["locations"][location] = bucket["locations"].get(location, 0) + 1
    ...
    for bucket in categories.values():
        ...
        top_location = max(
            bucket["locations"].items(),
            key=lambda pair: pair[1]
        )[0] if bucket["locations"] else "—"
        ...
        category_stats.append({
            ...
            "share": round(
                (bucket["count"] / total_products * 100) if total_products else 0,
                1
            )
        })

    category_stats.sort(key=lambda c: c["count"], reverse=True)
    ...
    recent_products = fetchall(
        """
        SELECT title, real_price, category, location, likes, created_at
        FROM products
        ORDER BY id DESC
        LIMIT 6
        """
    ...
    return jsonify({
        ...
        "recent": [dict(row) for row in recent_products]
    })
