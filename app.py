...
def api_groups_create():
    ...
    with CHAT_LOCK:
        ...
        _wallet_db_init(conn)
        cur = conn.execute(
            "INSERT INTO groups(name,owner_phone,member_limit,created_at) VALUES(?,?,?,?)",
            (name, phone, GROUP_MEMBER_LIMIT, now),
        )
        gid = cur.lastrowid
        conn.execute(
        ...
        conn.commit()
        conn.close()
    return jsonify({"success": True, "group_id": gid, "name": name, "message": "Group limeundwa! Shiriki link ya kujiunga na wanachama wako."})
...
def api_groups_mine():
    ...
    with CHAT_LOCK:
        conn = _analytics_db()
        _wallet_db_init(conn)
        owned = conn.execute("SELECT id,name,member_limit,created_at FROM groups WHERE owner_phone=?", (phone,)).fetchall()
        joined = conn.execute(
    ...
    with CHAT_LOCK:
        ...
        if not already:
            ...
            if count >= (grp["member_limit"] or GROUP_MEMBER_LIMIT):
                conn.close()
                return jsonify({"success": False, "message": "Group hili limejaa (limefikia ukomo wa wanachama)."}), 403
            conn.execute(
                "INSERT INTO group_members(group_id,phone,name,category,joined_at,active) VALUES(?,?,?,?,?,1)",
                (gid, phone, session.get("chat_name") or "Mwanachama", "Mwingine", now),
            )
        else:
            conn.execute("UPDATE group_members SET active=1 WHERE group_id=? AND phone=?", (gid, phone))
        conn.commit()
        conn.close()
    return jsonify({"success": True, "message": "Umejiunga na group!", "group_id": gid})
...
def api_chat_register():
    ...
    with CHAT_LOCK:
        ...
        conn.execute(
            """INSERT INTO chat_users(phone,name,mkoa,category,lat,lon,registered_at) VALUES(?,?,?,?,?,?,?)
            ...
            (phone, name, mkoa, category, lat, lon, now),
        )
        if is_new_signup:
...
def api_chat_messages_list():
    ...
    with CHAT_LOCK:
        ...
        _chat_db_init(conn)
        rows = conn.execute(
            """SELECT m.id,m.name,m.message,m.created_at,COALESCE(u.category,'Mwingine') AS category
...
def admin_page():
    ...
    if not session.get("is_admin"):
        if not ADMIN_PAGE_SECRET or request.args.get("key") != ADMIN_PAGE_SECRET:
