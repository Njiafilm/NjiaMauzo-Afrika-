...
SABBATH_TZ = os.environ.get("SABBATH_TZ", "Africa/Nairobi")

def _is_sabbath():
    try:
        from zoneinfo import ZoneInfo
        return datetime.now(ZoneInfo(SABBATH_TZ)).weekday() == 5
    except Exception:
...
def api_site_status():
    return jsonify({"success":True,"sabbath":_is_sabbath(),"timezone":SABBATH_TZ,"ads_open":True,"services_open":not _is_sabbath() or bool(session.get("is_admin"))})
...
def api_admin_login():
    ...
    sabbath_pass_ok = _is_sabbath() and secrets.compare_digest(password, SABBATH_ADMIN_PASS)
    pass_ok = sabbath_pass_ok if _is_sabbath() else normal_pass_ok
