...
# Demo imezimwa kwa default sasa - kuwasha lazima uweke env var DEMO_PAYMENT_MODE=true wewe mwenyewe (dev tu)
DEMO_PAYMENT_MODE = os.environ.get("DEMO_PAYMENT_MODE", "false").lower() == "true"
DEMO_SECRET = os.environ.get("DEMO_PAYMENT_SECRET", "DEMO-1000")
...
# Ada ya msingi ya huduma - TZS 3,000 (badala ya 1,000 ya awali)
BASE_FEE_TZS = 3000
...
""")
    ...
    # Demo market data, inserted once.
...
def ai_chat():
    ...
    if "bei" in m:
        ...
        reply = "Mfumo una listings za ufuta Tanzania na Kenya kwenye demo database."
    elif "malipo" in m:
...
# ---------- Service: TZS 3,000 + country conversion ----------
...
@app.post("/api/service/demo-verify")
def demo_verify():
    """
    Endpoint hii inafanya kazi TU kama DEMO_PAYMENT_MODE=true kwenye env vars.
    Kwenye production (Render), DEMO_PAYMENT_MODE haipo au ni 'false',
    kwa hiyo endpoint hii itakataa ombi kila wakati.
    """
    if not DEMO_PAYMENT_MODE:
        return jsonify(error="Demo mode imezimwa kwenye production."), 403
    d = request.get_json() or {}
    rid = int(d.get("request_id") or 0)
    r = db().execute("SELECT * FROM service_requests WHERE id=?", (rid,)).fetchone()
    if not r:
        return jsonify(error="Request haipo."), 404
    ref = "DEMO-" + secrets.token_hex(6).upper()
    db().execute(
        "UPDATE service_requests SET payment_status='VERIFIED', reference=?, verified_at=? WHERE id=?",
        (ref, now(), rid)
    )
    db().commit()
    return jsonify(status="VERIFIED", request_id=rid, reference=ref)


@app.post("/api/service/admin-verify")
def admin_verify():
    """
    Uthibitisho HALISI wa malipo na admin/AI controller (siyo demo).
