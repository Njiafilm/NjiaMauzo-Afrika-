...
# ===== ♻️ USED MARKET — Simu, Laptop, Tablet na Vifaa =====
USED_PRODUCT_CATEGORIES = {
    "simu": ["simu", "iphone", "samsung", "xiaomi", "tecno", "infinix", "oppo", "pixel"],
    "vifaa-vya-simu": ["charger", "chaja", "cable", "cover", "case", "power bank", "earbuds", "headset", "adapter", "screen"],
    "laptop": ["laptop", "computer mpakato", "macbook", "thinkpad", "dell", "hp", "lenovo", "asus", "acer"],
    "vifaa-vya-laptop": ["mouse", "keyboard", "ssd", "ram", "bag la laptop", "dock", "monitor", "hard disk"],
    "tablet": ["tablet", "ipad", "galaxy tab", "tab"],
    "vifaa-vya-tablet": ["tablet cover", "stylus", "pencil", "tablet keyboard", "tablet charger"],
}
USED_PRODUCT_ALLOWED_EXT = {".jpg", ".jpeg", ".png", ".webp", ".gif"}

def _used_products_db():
    conn = _analytics_db()
    conn.execute("""CREATE TABLE IF NOT EXISTS used_products (
        id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT,
        category TEXT NOT NULL DEFAULT 'simu', price_tzs INTEGER DEFAULT 0,
        image_url TEXT, condition TEXT DEFAULT 'used', condition_note TEXT,
        seller_name TEXT, seller_contact TEXT, supplier_url TEXT, source_country TEXT, location TEXT,
        verified INTEGER DEFAULT 0, active INTEGER DEFAULT 1, featured INTEGER DEFAULT 0,
        created_at TEXT NOT NULL
    )""")
    for col, ddl in [("supplier_url", "ALTER TABLE used_products ADD COLUMN supplier_url TEXT"),("source_country", "ALTER TABLE used_products ADD COLUMN source_country TEXT")]:
        try: conn.execute(f"SELECT {col} FROM used_products LIMIT 1")
        except sqlite3.OperationalError:
            try: conn.execute(ddl)
            except sqlite3.OperationalError: pass
    conn.execute("CREATE INDEX IF NOT EXISTS idx_used_category ON used_products(category)")
    conn.commit(); return conn

def _infer_used_category(title, description=""):
    text=f"{title or ''} {description or ''}".lower()
    for category, words in USED_PRODUCT_CATEGORIES.items():
        if any(w in text for w in words): return category
    return "simu"

def _seed_used_products():
    conn=_used_products_db()
    demo=[
        ("Samsung Galaxy S23 256GB","Simu used iliyoangaliwa vizuri; buyer akague kabla ya malipo.","simu",980000,"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=900&q=85","used-good","Hali nzuri • fingerprint/face unlock itathibitishwa","NjiaMauzo Verified Seller","","https://www.samsung.com/africa_en/smartphones/","South Korea","Dar es Salaam, Tanzania",1),
        ("iPhone 13 128GB","Used iPhone yenye hali nzuri; battery health itathibitishwa.","simu",1250000,"https://images.unsplash.com/photo-1592286927505-2fd0f9d6a0c5?w=900&q=85","used-good","Unlocked • battery health ithibitishwe","Verified Seller","","https://www.apple.com/shop/buy-iphone","USA","Dar es Salaam, Tanzania",1),
        ("HP EliteBook 840 G8","Laptop used kwa biashara, ofisi na masomo.","laptop",950000,"https://images.unsplash.com/photo-1496181133206-80ce9b88a853?w=900&q=85","used-good","Core i5 • RAM/SSD vitathibitishwa","Verified Seller","","https://www.hp.com/","USA","Arusha, Tanzania",1),
        ("Lenovo ThinkPad T14","Laptop used ya biashara yenye ports zilizokaguliwa.","laptop",1100000,"https://images.unsplash.com/photo-1517336714739-489689fd1ca8?w=900&q=85","used-good","Charger included • hali nzuri","Verified Seller","","https://www.lenovo.com/","China","Nairobi, Kenya",1),
        ("iPad 10th Generation","Tablet used kwa masomo, biashara na entertainment.","tablet",1150000,"https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?w=900&q=85","used-good","Wi-Fi • storage itathibitishwa","Verified Seller","","https://www.apple.com/ipad/","USA","Dubai, UAE",1),
        ("Anker Power Bank 20,000mAh","Kifaa used kilichokaguliwa; uwezo halisi uthibitishwe.","vifaa-vya-simu",75000,"https://images.unsplash.com/photo-1609592424850-7d3b5b8f0b5f?w=900&q=85","used-good","Charging ports zimekaguliwa","Verified Seller","","https://www.anker.com/","China","Dar es Salaam, Tanzania",1),
        ("Laptop SSD 512GB","SSD used kwa laptop/PC; health itaonyeshwa kabla ya kuuza.","vifaa-vya-laptop",95000,"https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?w=900&q=85","used-good","SMART health itathibitishwa","Verified Seller","","https://www.westerndigital.com/","USA","London, UK",1),
        ("iPad Keyboard + Cover","Vifaa vya tablet vilivyotumika, hali nzuri.","vifaa-vya-tablet",120000,"https://images.unsplash.com/photo-1585790050230-5dd28404ccb9?w=900&q=85","used-good","Keys na Bluetooth vitakaguliwa","Verified Seller","","https://www.apple.com/","USA","Dar es Salaam, Tanzania",1),
    ]
    now=datetime.utcnow().isoformat()+"Z"; count=conn.execute("SELECT COUNT(*) FROM used_products").fetchone()[0]
    if count==0:
        for title,desc,cat,price,img,cond,note,seller,contact,url,country,loc,verified in demo:
            conn.execute("INSERT INTO used_products(title,description,category,price_tzs,image_url,condition,condition_note,seller_name,seller_contact,supplier_url,source_country,location,verified,active,featured,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,?)",(title,desc,cat,price,img,cond,note,seller,contact,url,country,loc,verified,now))
    else:
        for title,desc,cat,price,img,cond,note,seller,contact,url,country,loc,verified in demo:
            conn.execute("UPDATE used_products SET image_url=COALESCE(NULLIF(image_url,''),?), supplier_url=COALESCE(supplier_url,?), source_country=COALESCE(source_country,?), location=COALESCE(NULLIF(location,''),?), seller_name=COALESCE(seller_name,?) WHERE title=?",(img,url,country,loc,seller,title))
    conn.commit(); conn.close()

def _used_products_list(q="", category=""):
    _seed_used_products(); conn=_used_products_db(); sql="SELECT * FROM used_products WHERE active=1"; params=[]
    cat=(category or "").strip().lower()
    if cat in set(USED_PRODUCT_CATEGORIES): sql += " AND category=?"; params.append(cat)
    rows=[dict(r) for r in conn.execute(sql+" ORDER BY featured DESC,id DESC",params).fetchall()]; conn.close()
    query=(q or "").strip().lower()
    if query:
        tokens=[t for t in re.split(r"\s+",query) if t]
        def hay(p): return " ".join(str(p.get(k) or "") for k in ("title","description","category","condition","location")).lower()
        rows=[p for p in rows if all(t in hay(p) for t in tokens)]
    unlocked=_is_unlocked() or bool(session.get("is_admin"))
    for p in rows:
        p["locked"]=not unlocked
        if not unlocked:
            p["price_tzs"]=None; p["seller_name"]=None; p["seller_contact"]=None; p["supplier_url"]=None
    return rows

@app.route("/api/used-products", methods=["GET"])
def api_used_products():
    return jsonify({"success":True,"products":_used_products_list(request.args.get("q") or "", request.args.get("category") or ""),
                    "categories":[{"id":k,"label":k.replace("-"," ").title()} for k in USED_PRODUCT_CATEGORIES]})

@app.route("/api/admin/used-products", methods=["GET","POST"])
def api_admin_used_products():
    if not session.get("is_admin"): return jsonify({"success":False,"message":"Si admin."}),403
    if request.method=="GET":
        conn=_used_products_db(); rows=[dict(r) for r in conn.execute("SELECT * FROM used_products WHERE active=1 ORDER BY featured DESC,id DESC").fetchall()]; conn.close()
        return jsonify({"success":True,"products":rows})
    d=request.form if request.form else (request.get_json(silent=True) or {}); title=(d.get("title") or "").strip()
    if not title: return jsonify({"success":False,"message":"Weka jina la bidhaa."}),400
    desc=(d.get("description") or "").strip(); requested=(d.get("category") or "").strip().lower(); category=requested if requested in USED_PRODUCT_CATEGORIES else _infer_used_category(title,desc)
    try: price=int(float(d.get("price_tzs") or 0))
    except: return jsonify({"success":False,"message":"Bei si sahihi."}),400
    image_url=(d.get("image_url") or "").strip(); f=request.files.get("image") if request.files else None
    if f and f.filename:
        ext=Path(f.filename).suffix.lower()
        if ext not in USED_PRODUCT_ALLOWED_EXT: return jsonify({"success":False,"message":"Picha lazima iwe JPG, PNG, WEBP au GIF."}),400
        dest=UPLOAD_DIR/"used"; dest.mkdir(parents=True,exist_ok=True); fname=f"used_{secrets.token_hex(8)}{ext}"; f.save(str(dest/fname)); image_url=f"/static/uploads/used/{fname}"
    try: verified=1 if str(d.get("verified") or "0").lower() in ("1","true","yes","ndio") else 0
    except: verified=0
    supplier_url=(d.get("supplier_url") or "").strip(); source_country=(d.get("source_country") or "").strip()
    conn=_used_products_db(); cur=conn.execute("INSERT INTO used_products(title,description,category,price_tzs,image_url,condition,condition_note,seller_name,seller_contact,supplier_url,source_country,location,verified,active,featured,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,1,0,?)",(title,desc,category,price,image_url,(d.get("condition") or "used").strip(),(d.get("condition_note") or "").strip(),(d.get("seller_name") or "").strip(),(d.get("seller_contact") or "").strip(),supplier_url,source_country,(d.get("location") or "").strip(),verified,datetime.utcnow().isoformat()+"Z")); conn.commit(); pid=cur.lastrowid; conn.close()
    return jsonify({"success":True,"message":"Used product imeongezwa.","product_id":pid,"category":category})

@app.route("/api/admin/used-products/<int:pid>", methods=["DELETE"])
def api_admin_used_product_delete(pid):
    if not session.get("is_admin"): return jsonify({"success":False,"message":"Si admin."}),403
    conn=_used_products_db(); conn.execute("UPDATE used_products SET active=0 WHERE id=?",(pid,)); conn.commit(); conn.close(); return jsonify({"success":True,"message":"Used product imeondolewa."})
...
def _ordinary_products_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS ordinary_products (
        ...
        featured INTEGER DEFAULT 0, seller_name TEXT, seller_contact TEXT, supplier_url TEXT, created_at TEXT NOT NULL)""")
    for col, ddl in [("seller_name", "ALTER TABLE ordinary_products ADD COLUMN seller_name TEXT"),("seller_contact", "ALTER TABLE ordinary_products ADD COLUMN seller_contact TEXT"),("supplier_url", "ALTER TABLE ordinary_products ADD COLUMN supplier_url TEXT")]:
        try: conn.execute(f"SELECT {col} FROM ordinary_products LIMIT 1")
        except sqlite3.OperationalError:
            try: conn.execute(ddl)
            except sqlite3.OperationalError: pass
...
def _seed_ordinary_demo_products():
    """Weka catalogue ya msingi; maelezo nyeti yanafunguliwa baada ya ada kuthibitishwa."""
    conn=_ordinary_products_db()
    demo=[
        ("Mfuko wa Kike wa Kahawia","Mfuko wa mtindo wa kila siku","mifuko","https://images.unsplash.com/photo-1584917865442-de89df76afd3?w=900&q=85",45000),
        ("Mfuko wa Kike wa Bluu","Seti ya mifuko ya mtindo","mifuko","https://images.unsplash.com/photo-1566150905458-1bf1fc113f0d?w=900&q=85",50000),
        ("Gauni la Kike la Kahawia","Gauni la mtindo wa kisasa","magauni","https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=900&q=85",55000),
        ("Mifuko ya Kike ya Pink","Seti ya mifuko ya kawaida","mifuko","https://images.unsplash.com/photo-1594223274512-ad4803739b7c?w=900&q=85",48000),
        ("Sandals za Rangi","Sandals za wanawake","viatu","https://images.unsplash.com/photo-1543163521-1bf539c55dd2?w=900&q=85",35000),
    ]
    now=datetime.utcnow().isoformat()+"Z"
    count=conn.execute("SELECT COUNT(*) FROM ordinary_products").fetchone()[0]
    if count==0:
        for title,desc,cat,img,price in demo:
            conn.execute("INSERT INTO ordinary_products(title,description,category,price_tzs,image_url,stock,active,featured,seller_name,seller_contact,supplier_url,created_at) VALUES(?,?,?,?,?,?,1,0,?,?,?,?)",(title,desc,cat,price,img,10,"NjiaMauzo Verified Seller","","https://www.jumia.co.tz/catalog/?q="+cat,now))
    else:
        for title,desc,cat,img,price in demo:
            conn.execute("UPDATE ordinary_products SET image_url=COALESCE(NULLIF(image_url,''),?), seller_name=COALESCE(seller_name,'NjiaMauzo Verified Seller') WHERE title=?",(img,title))
    conn.commit(); conn.close()
...
def api_admin_ordinary_products():
    ...
    seller_name=(d.get("seller_name") or "").strip(); seller_contact=(d.get("seller_contact") or "").strip(); supplier_url=(d.get("supplier_url") or "").strip()
    conn=_ordinary_products_db(); cur=conn.execute("INSERT INTO ordinary_products(title,description,category,price_tzs,image_url,stock,active,featured,seller_name,seller_contact,supplier_url,created_at) VALUES(?,?,?,?,?,?,1,0,?,?,?,?)",(title,desc,category,price,image_url,stock,seller_name,seller_contact,supplier_url,datetime.utcnow().isoformat()+"Z")); conn.commit(); pid=cur.lastrowid; conn.close()
...
def admin_room_page():
    """Admin Room iliyoboreshwa yenye Public Live Mirror; hailazimishi kubadilisha /admin ya awali."""
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Admin login inahitajika."}), 403
    admin_room_file = BASE_DIR / "admin_room.html"
    if admin_room_file.exists():
        return send_from_directory(BASE_DIR, "admin_room.html")
    return jsonify({"success": False, "message": "admin_room.html haipatikani."}), 404
