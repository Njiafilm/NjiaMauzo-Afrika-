...
def _is_sabbath_now():
    """Jumamosi kwa saa za Africa/Nairobi (EAT) = Sabato."""
    try:
        if ZoneInfo is not None:
            now = datetime.now(ZoneInfo("Africa/Nairobi"))
        else:
            now = datetime.utcnow()
        return now.weekday() == 5  # Monday=0 ... Saturday=5
    except Exception:
...
@app.route("/api/admin/sabbath-login", methods=["POST"])
def api_admin_sabbath_login():
    """Ufikiaji wa admin bila password — UNAFANYA KAZI SIKU YA SABATO PEKEE.
    Server ndiye anayeamua (si mteja), hivyo haiwezi kupitishwa nje ya Sabato."""
    if not _is_sabbath_now():
        return jsonify({"success": False, "message": "Ufikiaji huu unapatikana Sabato pekee."}), 403
    session["is_admin"] = True
    session["unlocked"] = True
    session["unlocked_until"] = datetime.utcnow().timestamp() + 24 * 3600
    session["user"] = {"email": "admin@njiamauzo.tz", "name": "Admin (Sabato)"}
    return jsonify({"success": True, "message": "Umeingia bila password — Sabato.", "admin_mode": True})
...
def api_bot_chat():
    ...
    with PRODUCTS_LOCK:
        ...
        recent = [p["title"] for p in SAMPLE_PRODUCTS[-5:]]
