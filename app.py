...
DEMO_PAYMENT_MODE = os.environ.get("DEMO_PAYMENT_MODE", "true").lower() == "true"
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS service_requests(
        ...
        country TEXT NOT NULL,
        phone TEXT,
        fee_tzs REAL NOT NULL DEFAULT 1000,
...
def ai_chat():
    ...
    if "bei" in m:
        ...
        reply = "Huduma ya kutafutiwa ni TZS 1,000. Kwenye TEST MODE unaweza kuthibitisha demo bila kutuma fedha."
    else:
...
# ---------- Service: TZS 1,000 + country conversion ----------
...
def service_fee():
    ...
    return jsonify(
        base_amount_tzs=1000,
        amount=round(1000 * rate, 2),
        currency=currency,
        country=country,
...
def service_start():
    ...
    cur = db().execute("""INSERT INTO service_requests
        ...
        VALUES(?,?,?,?,?,?, 'PENDING',?)""",
        (ref, q, country, 1000, currency, round(1000 * rate, 2), now()))
    db().commit()
...
def demo_verify():
    if not DEMO_PAYMENT_MODE:
        return jsonify(error="Demo mode imezimwa."), 403
    d = request.get_json() or {}
    rid = int(d.get("request_id") or 0)
    r = db().execute("SELECT * FROM service_requests WHERE id=?", (rid,)).fetchone()
    if not r:
...
def legacy_payment():
    return jsonify(
        reference="TEST-" + secrets.token_hex(5).upper(),
        status="PENDING",
        amount=10000,
        note="Legacy demo endpoint."
    )
