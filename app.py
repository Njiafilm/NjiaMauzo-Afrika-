...
SABBATH_ADMIN_PASS = os.environ.get("SABBATH_ADMIN_PASS", "sabbath-admin-change-me")
...
SUBSCRIPTION_PLANS = {
    "once": {
        "id": "once",
        "label_sw": "Mara moja (dakika 10)",
        "label_en": "One-time (10 minutes)",
        "seconds": 10 * 60,
        "multiplier": 1.0,
    },
    "daily": {
...
# Anzisha feed ikiwa na angalau bidhaa 16, na isiwe zaidi ya 20.
try:
    while len(SAMPLE_PRODUCTS) < 16:
        _generate_new_product()
except Exception:
    pass
...
def _is_sabbath_eat():
    try:
        return datetime.now(ZoneInfo("Africa/Nairobi")).weekday() == 5
    except Exception:
...
def api_admin_login():
    ...
    sabbath_pass_ok = secrets.compare_digest(password, SABBATH_ADMIN_PASS) if _is_sabbath_eat() else False
    pass_ok = sabbath_pass_ok if _is_sabbath_eat() else normal_pass_ok
    ...
    session["sabbath_admin"] = bool(_is_sabbath_eat() and sabbath_pass_ok and not normal_pass_ok)
    session["admin_ip"] = ip
    csrf = secrets.token_hex(24)
    session["csrf"] = csrf

    weak_default = ADMIN_PASS in ("0755248789", "njiamauzo2026", "admin", "password")
    return jsonify({
        "success": True,
        ...
        "csrf_token": csrf,
        "security_warning": (
            "BADILISHA password ya default! Weka ADMIN_PASS kwenye environment."
...
def api_admin_status():
    return jsonify({
        ...
        "sabbath": _is_sabbath_eat(),
        "sabbath_admin": bool(session.get("sabbath_admin")),
    })
...
def api_admin_change_password():
    ...
    return jsonify({
        "success": True,
        "message": "Nywila imebadilishwa kwa session hii. Weka ADMIN_PASS kwenye server kwa kudumu.",
    })
...
def api_bot_chat():
    ...
    if smart_reply:
        ...
        reply = ("Nimefikiria kuhusu bei. Bei kamili + eneo + muuzaji unapata baada ya "
                 "kulipa ada (subscription). Bofya «PATA MSAADA HAPA». "
...
ADVISORY_PLANS = {
    "1m": {"id":"1m","label_sw":"Dakika 1","label_en":"1 Minute","minutes":1,"seconds":60,"price_tzs":3000},
    "5m": {"id":"5m","label_sw":"Dakika 5","label_en":"5 Minutes","minutes":5,"seconds":300,"price_tzs":15000},
    "10m": {"id":"10m","label_sw":"Dakika 10","label_en":"10 Minutes","minutes":10,"seconds":600,"price_tzs":50000},
    "30m": {"id":"30m","label_sw":"Dakika 30","label_en":"30 Minutes","minutes":30,"seconds":1800,"price_tzs":100000},
}
ADVISORY_ORDERS = {}
...
def api_advisory_plans():
    ...
    for p in ADVISORY_PLANS.values():
        plans.append({**p,"amount":p["price_tzs"],"currency":"TZS","country":"Tanzania"})
    return jsonify({"success":True,"plans":plans,"country":"Tanzania"})
...
def api_advisory_request():
    ...
    with ADVISORY_LOCK:
        ADVISORY_ORDERS[order_id]={"status":"pending","method":data.get("njia") or "M-Pesa","phone":data.get("simu") or "","amount":amount,"currency":"TZS","country":"Tanzania","plan":plan["id"],"duration_minutes":plan["minutes"],"duration_seconds":plan["seconds"],"base_amount_tzs":plan["price_tzs"],"created":datetime.utcnow(),"session_uid":session.get("uid") or _client_ip()}
    session["pending_advisory_order_id"]=order_id
    return jsonify({"success":True,"order_id":order_id,"plan":plan["id"],"duration_minutes":plan["minutes"],"duration_seconds":plan["seconds"],"amount":amount,"currency":"TZS","country":"Tanzania"})
...
def api_payment_stk_status():
    ...
    if demo and checkout_id:
        ...
        return jsonify({
            ...
            "order_id": order_id,
            "demo": True,
            "message": "Subiri… weka PIN kwenye simu.",
            "waited": int(age),
        })
