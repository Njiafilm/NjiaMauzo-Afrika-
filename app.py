...
Live Activity 24/7 + AI Searcher + Smart Bot + Security + Paywall + SMS/WhatsApp notifications
"""
...
# Access / paywall
SERVICE_AMOUNT = int(os.environ.get("SERVICE_AMOUNT", "3000"))
ACCESS_TTL_HOURS = float(os.environ.get("ACCESS_TTL_HOURS", "24"))
...
# ---------------------------------------------------------------------------
def is_unlocked():
    """Admin ana unlock automatically. Wengine wanahitaji access ya kulipwa."""
    if is_admin():
        return True

    if session.get("access_unlocked") is True:
        exp = float(session.get("access_expires", 0) or 0)
        if exp > time.time():
            return True

        session.pop("access_unlocked", None)
        session.pop("access_expires", None)
        session.pop("access_method", None)
        session.pop("access_phone", None)
        session.pop("access_order", None)

    return False


def grant_access(hours=None, method="", phone="", order_id="", amount=None):
    """Fungua paywall kwa muda fulani baada ya malipo / confirmation."""
    if hours is None:
        hours = ACCESS_TTL_HOURS
    if amount is None:
        amount = SERVICE_AMOUNT

    expires = time.time() + float(hours) * 3600

    session["access_unlocked"] = True
    session["access_expires"] = expires
    session["access_method"] = method
    session["access_phone"] = phone
    session["access_order"] = order_id
    session["access_amount"] = amount
...
def access_payload():
    ...
    if admin:
        exp = float(session.get("admin_expires", 0) or 0)
        reason = "admin"
    elif unlocked:
        exp = float(session.get("access_expires", 0) or 0)
        reason = "paid"
    ...
    expires_in_sec = max(0, int(exp - now)) if exp else 0
    ...
    return {
        ...
        "reason": reason,
        "amount": SERVICE_AMOUNT,
        "ttl_hours": ACCESS_TTL_HOURS,
        "expires_at": datetime.fromtimestamp(exp, timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ") if exp else None,
        "expires_in_sec": expires_in_sec,
        "phone": session.get("access_phone", ""),
        "method": session.get("access_method", ""),
        "order_id": session.get("access_order", ""),
    }


# ---------------------------------------------------------------------------
# SMS / WHATSAPP NOTIFICATIONS
# TODO: Unganisha provider halisi hapa:
# SMS: Africa's Talking, Twilio, Beem, Selcom, Halotel API, etc.
# WhatsApp: Twilio WhatsApp API au Meta WhatsApp Cloud API
# ---------------------------------------------------------------------------
...
def send_sms_notification(to, message):
    """
    Placeholder ya SMS API.
    Badilisha hapa na provider wako halisi.
    """
    to = normalize_phone(to)
    logger.info("SMS TODO -> to=%s message=%s", to, message)

    # TODO: integrate real SMS provider
    # Example:
    # client.messages.create(to=to, body=message, from_=YOUR_SENDER_ID)
...
def send_whatsapp_notification(to, message):
    """
    Placeholder ya WhatsApp API.
    Badilisha hapa na provider wako halisi.
    """
    to = normalize_phone(to)
    link = f"https://wa.me/{to}?text={urllib.parse.quote(message)}" if to else None

    logger.info("WHATSAPP TODO -> to=%s message=%s", to, message)

    # TODO: integrate real WhatsApp API
    # Example:
    # requests.post(WHATSAPP_API_URL, json={...}, headers={...})

    return {
        "success": True,
        "channel": "whatsapp",
        "to": to,
        "status": "link_generated_todo",
        "link": link,
        "provider_response": None,
    }


def notify_payment_received(phone, method, order_id, amount, product_hint="bidhaa"):
    sms_msg = (
        f"NjiaMauzo Afrika: Malipo/ombi limepokelewa. Order {order_id}. "
        f"Huduma: taarifa za {product_hint}. Ada TZS {amount:,}. "
        f"WhatsApp: 0755248789."
    )
    ...
    wa_msg = (
        ...
        f"Njia: {method}\n"
        f"Ada: TZS {amount:,}\n"
        f"AI inaandaa taarifa za {product_hint}.\n\n"
        f"Tuma ujumbe hapa kuthibitisha malipo."
    )

    sms = send_sms_notification(phone, sms_msg)
    whatsapp = send_whatsapp_notification(phone, wa_msg)
...
def product_view(p, unlocked):
    title_full = p.get("title") or p.get("jina") or "Bidhaa"
    short_title = p.get("jina") or title_full.split(" — ")[0].strip()
    image = p.get("image") or PRODUCT_IMAGE_MAP.get(p.get("jina") or short_title, DEFAULT_PRODUCT_IMAGE)
    ...
    base = {
        ...
        "likes": p.get("likes", 0),
        "full_access": bool(unlocked),
    }

    if unlocked:
        base.update({
            "title": title_full,
            "jina": p.get("jina", short_title),
...
def get_products():
    unlocked = is_unlocked()
    products = [product_view(p, unlocked) for p in products_store[:30]]
    return jsonify({
        "success": True,
        "products": products,
        "admin_mode": is_admin(),
...
def ai_products():
    ...
    unlocked = is_unlocked()
    public_results = [product_view(p, unlocked) for p in results]

    return jsonify({
        "success": True,
        "products": public_results,
        "admin_mode": is_admin(),
        "unlocked": unlocked,
    })
...
def research():
    ...
    if not unlocked:
        return jsonify({
            "success": True,
            "locked": True,
            "message": "Lipa ada TZS 3,000 ili kupata uchambuzi kamili wa bei, maeneo na vyanzo.",
            "summary": {
                "query": query,
...
def market_stats():
    ...
    for i, (loc, cnt) in enumerate(sorted(locations.items(), key=lambda x: -x[1])[:10]):
        top_locations.append({
            "location": loc if unlocked else f"Eneo #{i+1}",
            "count": cnt,
        })

    recent = [
        {
            "title": p.get("jina") or (p.get("title") or "").split(" — ")[0].strip(),
            "location": p.get("location") if unlocked else "",
        }
        for p in sorted(products_store, key=lambda x: x.get("id", 0), reverse=True)[:8]
    ]
...
def payment_request():
    ...
    method = str(data.get("njia", "")).strip()[:40]
    phone_raw = str(data.get("simu", "")).strip()
    phone = normalize_phone(phone_raw)

    try:
        amount = int(data.get("amount", SERVICE_AMOUNT))
    except (TypeError, ValueError):
        amount = SERVICE_AMOUNT

    if not method or len(phone) < 9:
        return jsonify({"success": False, "message": "Jaza njia ya malipo na namba ya simu sahihi."}), 400
    ...
    sample = None
    if _private_products:
        sample = random.choice(list(_private_products.values()))
    else:
        batch = ai_search_products(limit=1)
        if batch:
            sample = batch[0]

    product_hint = sample.get("jina", "bidhaa") if sample else "bidhaa"

    # Kwa demo: tunafungua access mara moja baada ya request.
    # Katika production, unaweza kusubiri webhook / confirmation ya provider.
    auto_unlock = os.environ.get("AUTO_UNLOCK_AFTER_PAYMENT_REQUEST", "1").lower() not in ("0", "false", "no")

    if auto_unlock:
        expires = grant_access(hours=ACCESS_TTL_HOURS, method=method, phone=phone, order_id=order_id, amount=amount)
    ...
    notifications = notify_payment_received(phone, method, order_id, amount, product_hint)
    ...
    return jsonify({
        ...
        "order_id": order_id,
        "status": status,
        "unlocked": auto_unlock,
        "expires_at": datetime.fromtimestamp(expires, timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ") if expires else None,
        "expires_in_sec": max(0, int(expires - time.time())) if expires else 0,
        "notifications": notifications,
        "whatsapp_link": notifications.get("whatsapp", {}).get("link"),
        "hint": f"AI itaandaa taarifa za {product_hint} baada ya kuthibitisha malipo.",
        "message": "Malipo/ombi limepokelewa. Taarifa kamili zimefunguliwa kwa muda uliowekwa." if auto_unlock else "Ombi limepokelewa. Subiri uthibitisho wa malipo.",
    })
...
def payment_confirm():
    ...
    if phone and order.get("phone") and normalize_phone(order["phone"]) != phone:
        return jsonify({"success": False, "message": "Namba ya simu hailingani na order."}), 400

    expires = grant_access(
        hours=ACCESS_TTL_HOURS,
        method=order.get("method", ""),
        phone=order.get("phone", ""),
        order_id=order_id,
        amount=order.get("amount", SERVICE_AMOUNT),
    )
    ...
    notifications = notify_payment_received(
        ...
        order.get("method", ""),
        order_id,
        order.get("amount", SERVICE_AMOUNT),
        order.get("product_hint", "bidhaa"),
    )

    push_activity(
        f"✅ Malipo yamethibitishwa — Order {order_id}. Access imefunguliwa.",
        activity_type="payment",
    )
    ...
    return jsonify({
        ...
        "expires_in_sec": max(0, int(expires - time.time())),
        "notifications": notifications,
        "whatsapp_link": notifications.get("whatsapp", {}).get("link"),
        "message": "Malipo yamethibitishwa. Access ya taarifa kamili imefunguliwa.",
    })
...
def register():
    data = request.get_json(force=True, silent=True) or {}
    session["user"] = {
        "name": str(data.get("name", "Mtumiaji"))[:60],
...
logger.info("Live Activity 24/7 + Smart Bot + Paywall started")
