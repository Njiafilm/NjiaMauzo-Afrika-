...
ADMIN_TEMP_OPEN_UNTIL = os.environ.get(
    "ADMIN_TEMP_OPEN_UNTIL", "2026-08-20T21:49:00Z"
).strip()
...
def api_chat_register():
    ...
    with CHAT_LOCK:
        ...
        _chat_db_init(conn)
        conn.execute(
            """INSERT INTO chat_users(phone,name,mkoa,lat,lon,registered_at) VALUES(?,?,?,?,?,?)
            ...
            (phone, name, mkoa, lat, lon, now),
        )
        conn.commit()
        conn.close()
    session["phone"] = phone
    session["chat_name"] = name
    return jsonify({"success": True, "message": "Umesajiliwa. Sasa unaweza kuandika kwenye majadiliano.", "phone": phone, "name": name})
...
def api_chat_messages_list():
    ...
    with CHAT_LOCK:
        ...
        _chat_db_init(conn)
        rows = conn.execute(
            "SELECT id,name,message,created_at FROM chat_messages WHERE status='ok' ORDER BY id DESC LIMIT 50"
        ).fetchall()
        conn.close()
    return jsonify({
        "success": True,
...
def api_chat_send():
    ...
    allowed, reason = _sabbath_message_allowed(message)
    now = datetime.utcnow().isoformat() + "Z"
    ...
    with CHAT_LOCK:
        ...
        _chat_db_init(conn)
        conn.execute(
            "INSERT INTO chat_messages(phone,name,message,status,reject_reason,created_at) VALUES(?,?,?,?,?,?)",
...
def api_admin_room_snapshot():
    ...
    with ADS_LOCK:
        ads_active = [a for a in ADS_STORE if _ad_is_on_air(a)]
...
# Seed demo ad — published + always-on (no schedule) so public sees welcome marquee
ADS_STORE.append({
    "id": 0,
...
def _parse_ad_dt(value):
    """Parse ISO datetime string to aware UTC datetime, or None."""
    if not value:
        return None
    try:
...
def _ad_is_on_air(ad, now=None):
    """Tangazo linaonekana kwa umma tu kama: published + active + ndani ya muda (kama umepangwa)."""
    if not ad:
        return False
    if ad.get("published") is False:
        return False
    if ad.get("active") is False:
        return False
    now = now or datetime.now(timezone.utc)
    starts = _parse_ad_dt(ad.get("starts_at"))
    ends = _parse_ad_dt(ad.get("ends_at"))
    # Ikiwa duration_minutes pekee ilitolewa wakati wa publish bila ends_at, tumia created+duration
    if ends is None and ad.get("duration_minutes") and ad.get("published_at"):
        pub = _parse_ad_dt(ad.get("published_at"))
        if pub:
            try:
                ends = pub + timedelta(minutes=int(ad["duration_minutes"]))
            except Exception:
                ends = None
...
def _ads_on_air():
    with ADS_LOCK:
        return [a for a in ADS_STORE if _ad_is_on_air(a)]


def _normalize_ad_schedule(data):
    """Extract starts_at / ends_at / duration_minutes from form or JSON."""
    starts_at = (data.get("starts_at") or data.get("start_at") or "").strip() or None
    ends_at = (data.get("ends_at") or data.get("end_at") or "").strip() or None
    duration_minutes = data.get("duration_minutes") or data.get("duration") or data.get("muda_dakika")
    try:
        duration_minutes = int(duration_minutes) if duration_minutes not in (None, "", "null") else None
    except Exception:
        duration_minutes = None
    if duration_minutes is not None and duration_minutes <= 0:
        duration_minutes = None
    # Ikiwa starts_at + duration pekee → hesabu ends_at
    if starts_at and duration_minutes and not ends_at:
        st = _parse_ad_dt(starts_at)
        if st:
            ends_at = (st + timedelta(minutes=duration_minutes)).isoformat().replace("+00:00", "Z")
    return starts_at, ends_at, duration_minutes


@app.route("/api/ads", methods=["GET"])
def api_ads_list():
    """Umma: matangazo yaliyo ON AIR tu (published + active + ndani ya muda)."""
    active = _ads_on_air()
    with ADS_LOCK:
        counts = dict(AD_ENGAGE)
    return jsonify({"success": True, "ads": active, "counts": counts, "on_air_count": len(active)})
...
def api_admin_ads_create():
    """
    Unda tangazo moja au hadi 5 kwa wakati mmoja.
    - published=false → haitoki hewani mpaka admin achapishe (publish)
    - starts_at / ends_at / duration_minutes → muda maalumu wa kuwa hewani
    - files[] au file — hadi faili 5
    """
    global _next_ad_id
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    ...
    def _one_ad(title, media_type, media_url, link_url, marquee, active, fullscreen,
                published, starts_at, ends_at, duration_minutes):
        global _next_ad_id
        ...
        with ADS_LOCK:
            ...
            ad = {
                ...
                "published_at": now_iso if published else None,
                "created": now_iso,
            }
            ADS_STORE.append(ad)
    ...
    if is_multipart:
        base_title = (request.form.get("title") or "").strip() or "Tangazo"
        ...
        # Default: draft (published=false) unless admin explicitly publishes
        published = (request.form.get("published") or "0") in ("1", "true", "True", "yes")
        starts_at, ends_at, duration_minutes = _normalize_ad_schedule(request.form)

        files = request.files.getlist("files") or []
        single = request.files.get("file")
        if single and single.filename and single not in files:
            files.append(single)
        # Also support file0..file4
        for i in range(5):
            f = request.files.get(f"file{i}")
            if f and f.filename and f not in files:
                files.append(f)
        ...
        if files:
            for idx, f in enumerate(files):
                ...
                if mt not in ("video", "audio", "image"):
                    if ext in (".mp4", ".webm", ".mov"):
                        ...
                        mt = "image"
                title = base_title if len(files) == 1 else f"{base_title} ({idx+1})"
                ad = _one_ad(title, mt, media_url, link_url, marquee if idx == 0 else "",
                             active, fullscreen, published, starts_at, ends_at, duration_minutes)
                if ad:
                    created.append(ad)
        else:
            ...
            if media_url and media_type == "text":
                media_type = "link"
            ad = _one_ad(base_title, media_type, media_url, link_url, marquee,
                         active, fullscreen, published, starts_at, ends_at, duration_minutes)
            if ad:
                created.append(ad)
    else:
        data = request.get_json(silent=True) or {}
        # Bulk JSON: { "ads": [ {...}, ... ] } max 5
        items = data.get("ads") if isinstance(data.get("ads"), list) else [data]
        items = items[:5]
        for item in items:
            if not isinstance(item, dict):
                continue
            title = (item.get("title") or "").strip() or "Tangazo"
            media_type = (item.get("type") or "link").strip().lower()
            media_url = (item.get("media_url") or item.get("url") or "").strip()
            link_url = (item.get("link_url") or "").strip()
            marquee = (item.get("marquee") or "").strip()
            active = item.get("active", True) is not False
            fullscreen = item.get("fullscreen", True) is not False
            published = item.get("published", False) is True
            starts_at, ends_at, duration_minutes = _normalize_ad_schedule(item)
            ad = _one_ad(title, media_type, media_url, link_url, marquee,
                         active, fullscreen, published, starts_at, ends_at, duration_minutes)
            if ad:
                created.append(ad)
    ...
    on_air = sum(1 for a in created if _ad_is_on_air(a))
    msg = f"{len(created)} tangazo limehifadhiwa."
    if on_air:
        msg += f" {on_air} lipo hewani sasa."
    else:
        msg += " Haijatoka hewani — chapisha (Publish) au subiri muda wa kuanza."
    return jsonify({
        "success": True,
        "ads": created,
        "ad": created[0],
...
def api_admin_ads_publish(aid):
    """Chapisha tangazo (published=true) na/au sasisha muda wa kuwa hewani."""
    if not session.get("is_admin"):
        return jsonify({"success": False, "message": "Si admin."}), 403
    ok, err = _require_csrf()
    if not ok:
        return err
    data = request.get_json(silent=True) or {}
    starts_at, ends_at, duration_minutes = _normalize_ad_schedule(data)
    with ADS_LOCK:
        for a in ADS_STORE:
            if a.get("id") == aid:
                a["published"] = True
                a["active"] = True
                a["published_at"] = datetime.utcnow().isoformat() + "Z"
                ...
                if duration_minutes is not None:
                    ...
                    # Ikiwa hakuna ends_at, hesabu kutoka sasa
                    if not a.get("ends_at") and not starts_at:
                        end = datetime.now(timezone.utc) + timedelta(minutes=duration_minutes)
                        a["ends_at"] = end.isoformat().replace("+00:00", "Z")
                        a["starts_at"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
                return jsonify({
                    "success": True,
                    "ad": a,
                    "on_air": _ad_is_on_air(a),
                    "message": "Tangazo limechapishwa." if _ad_is_on_air(a) else "Limechapishwa — litasubiri muda wa kuanza.",
                })
    return jsonify({"success": False, "message": "Tangazo halipatikani."}), 404
...
# ===== VIDEO CALL (WebRTC signaling via REST) =====
# Baada ya kujisajili kwenye chat, watumiaji wanaweza kupiga video call
# kwa ndugu/jamaa waliojisajili (signaling: offer/answer/ICE).
VIDEO_CALL_LOCK = threading.Lock()
VIDEO_CALL_ROOMS = {}  # room_id -> {id, from_phone, to_phone, offer, answer, ice_from, ice_to, status, created}
VIDEO_CALL_TTL = 600  # sekunde 10


def _cleanup_video_rooms():
    now = time.time()
    with VIDEO_CALL_LOCK:
        dead = [k for k, v in VIDEO_CALL_ROOMS.items()
                if now - float(v.get("created_ts") or 0) > VIDEO_CALL_TTL or v.get("status") == "ended"]
        for k in dead:
            VIDEO_CALL_ROOMS.pop(k, None)


@app.route("/api/chat/users/online", methods=["GET"])
def api_chat_users_online():
    """Orodha ya watumiaji waliojisajili (kwa kuita video call)."""
    phone = session.get("phone")
    if not phone:
        return jsonify({"success": False, "message": "Jisajili kwanza kwa namba ya simu."}), 403
    with CHAT_LOCK:
        conn = _analytics_db()
        _chat_db_init(conn)
        rows = conn.execute(
            "SELECT phone,name,mkoa,registered_at FROM chat_users ORDER BY registered_at DESC LIMIT 80"
        ).fetchall()
        conn.close()
    users = [dict(r) for r in rows if str(r["phone"]) != str(phone)]
    return jsonify({"success": True, "me": phone, "users": users})


@app.route("/api/chat/call/create", methods=["POST"])
def api_chat_call_create():
    """Anzisha video call room kwa kuwaalika rafiki (namba)."""
    _cleanup_video_rooms()
    phone = session.get("phone")
    if not phone:
        return jsonify({"success": False, "message": "Jisajili kwanza ili upige video call."}), 403
    if _is_banned_identifier(phone):
        return jsonify({"success": False, "message": "Namba imezuiwa."}), 403
    data = request.get_json(silent=True) or {}
    to_phone = _normalize_phone(data.get("to_phone") or data.get("phone") or "")
    if not to_phone or not _valid_tz_style_phone(to_phone):
        return jsonify({"success": False, "message": "Weka namba sahihi ya rafiki (mfano 0755xxxxxx)."}), 400
    if to_phone == phone:
        return jsonify({"success": False, "message": "Huwezi kujipigia mwenyewe."}), 400
    room_id = secrets.token_hex(8)
    with VIDEO_CALL_LOCK:
        VIDEO_CALL_ROOMS[room_id] = {
            "id": room_id,
            "from_phone": phone,
            "from_name": session.get("chat_name") or "Mtumiaji",
            "to_phone": to_phone,
            "offer": None,
            "answer": None,
            "ice_from": [],
    ...
    return jsonify({
        "success": True,
        "room_id": room_id,
        "message": "Simu inapigwa… subiri jibu.",
        "room": {"id": room_id, "to_phone": to_phone, "status": "ringing"},
    })


@app.route("/api/chat/call/incoming", methods=["GET"])
def api_chat_call_incoming():
    """Angalia kama kuna video call inayokuja kwako."""
    _cleanup_video_rooms()
    phone = session.get("phone")
    if not phone:
    ...
    with VIDEO_CALL_LOCK:
        calls = [
            ...
            for r in VIDEO_CALL_ROOMS.values()
            if r.get("to_phone") == phone and r.get("status") in ("ringing", "connected")
        ]
    return jsonify({"success": True, "calls": calls})


@app.route("/api/chat/call/<room_id>", methods=["GET"])
def api_chat_call_get(room_id):
    phone = session.get("phone")
    if not phone:
        return jsonify({"success": False, "message": "Jisajili kwanza."}), 403
    with VIDEO_CALL_LOCK:
        room = VIDEO_CALL_ROOMS.get(room_id)
        if not room:
            return jsonify({"success": False, "message": "Room haipatikani."}), 404
        if phone not in (room.get("from_phone"), room.get("to_phone")):
            return jsonify({"success": False, "message": "Huruhusiwi."}), 403
        # ICE for this peer
        if phone == room.get("from_phone"):
            ice = list(room.get("ice_to") or [])
            room["ice_to"] = []
        else:
            ice = list(room.get("ice_from") or [])
            room["ice_from"] = []
        payload = {
            "success": True,
            "room_id": room_id,
            "status": room.get("status"),
            "from_phone": room.get("from_phone"),
            "to_phone": room.get("to_phone"),
            "from_name": room.get("from_name"),
...
@app.route("/api/chat/call/<room_id>/signal", methods=["POST"])
def api_chat_call_signal(room_id):
    """Tuma WebRTC offer / answer / ice candidate."""
    phone = session.get("phone")
    if not phone:
        return jsonify({"success": False, "message": "Jisajili kwanza."}), 403
    data = request.get_json(silent=True) or {}
    kind = (data.get("type") or "").lower()  # offer | answer | ice
    with VIDEO_CALL_LOCK:
        room = VIDEO_CALL_ROOMS.get(room_id)
        if not room:
            return jsonify({"success": False, "message": "Room haipatikani."}), 404
        if phone not in (room.get("from_phone"), room.get("to_phone")):
            return jsonify({"success": False, "message": "Huruhusiwi."}), 403
        if kind == "offer":
            room["offer"] = data.get("sdp") or data.get("offer")
            room["status"] = "ringing"
        elif kind == "answer":
            room["answer"] = data.get("sdp") or data.get("answer")
            room["status"] = "connected"
        elif kind == "ice":
            cand = data.get("candidate")
            if cand:
                if phone == room.get("from_phone"):
                    room.setdefault("ice_from", []).append(cand)
                else:
                    room.setdefault("ice_to", []).append(cand)
        else:
            return jsonify({"success": False, "message": "type lazima: offer, answer, au ice."}), 400
    return jsonify({"success": True, "message": "Signal imehifadhiwa."})


@app.route("/api/chat/call/<room_id>/end", methods=["POST"])
def api_chat_call_end(room_id):
    phone = session.get("phone")
    with VIDEO_CALL_LOCK:
        room = VIDEO_CALL_ROOMS.get(room_id)
        if room and phone in (room.get("from_phone"), room.get("to_phone"), None):
            room["status"] = "ended"
    return jsonify({"success": True, "message": "Simu imekatwa."})


@app.route("/api/chat/call/<room_id>/accept", methods=["POST"])
def api_chat_call_accept(room_id):
    phone = session.get("phone")
    if not phone:
        return jsonify({"success": False, "message": "Jisajili kwanza."}), 403
    with VIDEO_CALL_LOCK:
        room = VIDEO_CALL_ROOMS.get(room_id)
        if not room or room.get("to_phone") != phone:
            return jsonify({"success": False, "message": "Simu haipatikani."}), 404
        if room.get("status") == "ended":
            return jsonify({"success": False, "message": "Simu imeisha."}), 400
        room["status"] = "connected"
    return jsonify({"success": True, "room_id": room_id, "message": "Umekubali simu."})
