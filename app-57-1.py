...
ADMIN_TEMP_OPEN_UNTIL = os.environ.get(
    "ADMIN_TEMP_OPEN_UNTIL", "2026-08-23T20:58:00Z"
).strip()
...
def api_group_send(gid):
    ...
    attachment_url = (data.get("attachment_url") or "").strip()[:300]
...
def api_chat_register():
    ...
    avatar_url = (data.get("avatar_url") or "").strip()[:300]
...
def _security_headers(resp):
    # Performance: compress sizeable text/JSON responses when the client supports gzip.
    # This reduces transfer size on mobile/slow networks without changing API payloads.
    try:
        accept = (request.headers.get("Accept-Encoding") or "").lower()
        ctype = (resp.headers.get("Content-Type") or "").lower()
        if ("gzip" in accept and "gzip" not in (resp.headers.get("Content-Encoding") or "").lower()
                and not resp.direct_passthrough and len(resp.get_data()) >= 700
                and any(x in ctype for x in ("application/json", "text/html", "text/css", "javascript"))):
            raw = resp.get_data()
