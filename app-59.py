...
def api_admin_logout():
    try:
        ...
        conn.execute("UPDATE chat_admin_presence SET active=0,updated_at=? WHERE id=1", (datetime.utcnow().isoformat()+"Z",))
        conn.commit(); conn.close()
    except Exception:
        pass
    session.clear()
...
def _chat_db_init(conn):
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS chat_admin_presence (
        id INTEGER PRIMARY KEY CHECK (id=1),
        active INTEGER NOT NULL DEFAULT 0,
        updated_at TEXT NOT NULL
    )""")
    conn.execute("INSERT OR IGNORE INTO chat_admin_presence(id,active,updated_at) VALUES(1,0,?)", (datetime.utcnow().isoformat()+"Z",))
    conn.execute("CREATE INDEX IF NOT EXISTS idx_chat_msg_created ON chat_messages(created_at)")
    conn.execute("""CREATE TABLE IF NOT EXISTS chat_bot_state (
        phone TEXT PRIMARY KEY,
        waiting_sent INTEGER DEFAULT 0,
        active_sent INTEGER DEFAULT 0,
        updated_at TEXT NOT NULL
    )""")
...
def _chat_admin_presence(conn=None):
    own = conn is None
    if own:
        conn = _analytics_db()
        _chat_db_init(conn)
    row = conn.execute("SELECT active,updated_at FROM chat_admin_presence WHERE id=1").fetchone()
    if own:
        conn.close()
    return bool(row and row["active"]), (row["updated_at"] if row else None)


def _chat_insert_bot(conn, phone, message):
    now = datetime.utcnow().isoformat() + "Z"
    conn.execute(
        "INSERT INTO chat_messages(phone,name,message,status,created_at) VALUES(?,?,?,?,?)",
        ("BOT", "NjiaMauzo Bot", message[:500], "ok", now),
    )


def _chat_bot_after_user_message(conn, phone, name, message):
    """Bot wa huduma: hukusanya tatizo + eneo, kisha husubiri Admin."""
    user = conn.execute("SELECT mkoa FROM chat_users WHERE phone=?", (phone,)).fetchone()
    location = (user["mkoa"] if user else "") or ""
    state = conn.execute("SELECT phone,waiting_sent,active_sent FROM chat_bot_state WHERE phone=?", (phone,)).fetchone()
    if not state:
        conn.execute("INSERT INTO chat_bot_state(phone,waiting_sent,active_sent,updated_at) VALUES(?,0,0,?)", (phone, datetime.utcnow().isoformat()+"Z"))
        state = conn.execute("SELECT phone,waiting_sent,active_sent FROM chat_bot_state WHERE phone=?", (phone,)).fetchone()
        _chat_insert_bot(conn, phone, "KARIBU NJIAMauzo Afrika. 🤖 Tafadhali taja changamoto yako na eneo/mkoa unaloishi ili tukusaidie.")
    if not location:
        # Usitoe ujumbe wa kusubiri mpaka eneo litajwe; mteja aendelee na hatua ya eneo.
        conn.execute("UPDATE chat_bot_state SET updated_at=? WHERE phone=?", (datetime.utcnow().isoformat()+"Z", phone))
        return
    active, _ = _chat_admin_presence(conn)
    if active:
        if not state["active_sent"]:
            _chat_insert_bot(conn, phone, "SASA TUNAEWZA KUKUSAIDIA WAHUDUMU WETU WAKO TAYARI,KARIBU.")
            conn.execute("UPDATE chat_bot_state SET active_sent=1,updated_at=? WHERE phone=?", (datetime.utcnow().isoformat()+"Z", phone))
    else:
        if not state["waiting_sent"]:
            _chat_insert_bot(conn, phone, "SAMAHANI SANA NDUGU MTEJA, WAHUDUMU WANAHUDUMIA WENGINE TAFADHALI ENDELEA KUSUBILI AU JARIBU TENA BAADAYE,")
            conn.execute("UPDATE chat_bot_state SET waiting_sent=1,updated_at=? WHERE phone=?", (datetime.utcnow().isoformat()+"Z", phone))


def _chat_bot_broadcast_active(conn):
    active, _ = _chat_admin_presence(conn)
    if not active:
        return
    users = conn.execute("SELECT phone FROM chat_bot_state WHERE waiting_sent=1 AND active_sent=0").fetchall()
    for u in users:
        _chat_insert_bot(conn, u["phone"], "SASA TUNAEWZA KUKUSAIDIA WAHUDUMU WETU WAKO TAYARI,KARIBU.")
        conn.execute("UPDATE chat_bot_state SET active_sent=1,updated_at=? WHERE phone=?", (datetime.utcnow().isoformat()+"Z", u["phone"]))


@app.route("/api/chat/admin-status", methods=["GET"])
def api_chat_admin_status():
    conn = _analytics_db(); _chat_db_init(conn)
    active, updated = _chat_admin_presence(conn)
    conn.close()
    return jsonify({"success": True, "active": active, "updated_at": updated})
...
def api_admin_chat_presence():
    ...
    conn = _analytics_db(); _chat_db_init(conn)
    now = datetime.utcnow().isoformat()+"Z"
    conn.execute("UPDATE chat_admin_presence SET active=?,updated_at=? WHERE id=1", (1 if active else 0, now))
    if active:
        _chat_bot_broadcast_active(conn)
    conn.commit(); conn.close()
    return jsonify({"success": True, "active": active, "message": "Admin yuko active." if active else "Admin hayuko active."})
...
def api_chat_send():
    ...
    with CHAT_LOCK:
        ...
        if allowed:
            _chat_bot_after_user_message(conn, phone, name, message)
        conn.commit()
        conn.close()
    with CHAT_TYPING_LOCK:
